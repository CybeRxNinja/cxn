...
2. Run `gh search prs` for issue number. Reasonable existing PR → review per `.cxn/commands/review-prs.md`, report `existing-pr`; do NOT create competing fix.
...
WT="$HOME/.cxn/wt/${ENC}/fix-issue-<N>"
...
Worktree:  ~/.cxn/wt/.../fix-issue-<N>            (if created)
