...
`scripts/probe.ts`: `@cyberxninja-omp/pi-ai` `completeSimple`; production-matching model/auth/provider behavior.
...
```ts
import { arkToWireSchema } from "@cyberxninja-omp/pi-ai"; // or toolWireSchema(tool)
