...
function Configure-BashShell {
    try {
        ...
        if ($bashPath) {
            ...
            Write-Host ""
            Write-Host "No bash shell found - CXN will use its built-in shell." -ForegroundColor Cyan
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("cxn-install-" + [System.Guid]::NewGuid().ToString("N"))
