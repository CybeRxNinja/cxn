#!/bin/sh
# Install the dev `cxn` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/cxn`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/cxn` to `/cxn` and try to
# write under `/` — see https://github.com/can1357/oh-my-pi/issues/3701.
set -e

repo_root=$(CDPATH='' cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/cxn

if [ ! -x "$target" ]; then
	echo "link-cxn: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/cxn"
echo "link-cxn: linked $global_bin/cxn -> $target"
