...
(`~/.cxn/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.cxn/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.cxn/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@cxn/pi-ai`,
credentials resolved through cxn's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.cxn/stats-audit-cache.json` (keyed by session id +
