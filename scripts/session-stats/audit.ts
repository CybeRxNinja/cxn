#!/usr/bin/env bun
/**
 * Token-usage audit over the local cxn session corpus (~/.cxn/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @cxn/pi-ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through cxn's auth
 * storage (~/.cxn/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@cxn/pi-utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".cxn", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".cxn", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.cxn/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.cxn/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (cxn login or env var required)`);
	}
	return { model, apiKey };
}
