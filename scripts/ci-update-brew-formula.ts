...
export function renderFormula(version: string, sums: Record<string, string>): string {
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/cxn-darwin-arm64",
          using: :nounzip
      sha256 "${sums["cxn-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/cxn-darwin-x64",
          using: :nounzip
      sha256 "${sums["cxn-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/cxn-linux-arm64",
          using: :nounzip
      sha256 "${sums["cxn-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/cxn-linux-x64",
          using: :nounzip
      sha256 "${sums["cxn-linux-x64"]}"
    end
  end
...
async function main(): Promise<void> {
	...
	const targets = ["cxn-darwin-arm64", "cxn-darwin-x64", "cxn-linux-arm64", "cxn-linux-x64"];
