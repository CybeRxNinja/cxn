...
**Catalog import convention**: code in this repo imports catalog _values_ (bundled models, model-thinking helpers, identity, descriptors, model manager/cache) from `@cyberxninja-omp/pi-catalog/<module>` — never via `@cyberxninja-omp/pi-ai`. The pi-ai barrel re-exports only the model/effort _types_ its own signatures use (`Model`, `Api`, `ThinkingConfig`, `Effort`, …); type-only imports of those from `@cyberxninja-omp/pi-ai` are fine.
...
- **Worker scripts**: workers re-enter the CLI entrypoint; never spawn separate worker entry modules. `cli.ts` declares itself as the worker host at startup (`declareWorkerHostEntry()` from `@cyberxninja-omp/pi-utils/env`) and dispatches hidden argv selectors (`__cxn_worker_stats_sync`, `__cxn_worker_tab`, `__cxn_worker_js_eval`, `__cxn_worker_tiny_inference`) before loading the command registry. Spawn sites use:
  ```ts
  import { workerHostEntry } from "@cyberxninja-omp/pi-utils";
...
| Binary lookup   | `$which("git")` from `@cyberxninja-omp/pi-utils` | `spawnSync(["which", "git"])`      |
...
- **Tabs → spaces** via `replaceTabs()` (from `@cyberxninja-omp/pi-tui` or `../tools/render-utils`).
