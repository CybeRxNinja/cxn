...
- **Scale set** (`cxn-kata` release, ns `arc-runners`) - the `AutoscalingRunnerSet`
...
**Scale set** (`cxn-kata`), using the runner cache PVC and values file from step 3:
```bash
helm install cxn-kata \
  --namespace arc-runners --create-namespace \
  --version 0.14.2 \
  -f arc-cxn-values.yaml \
...
# cxn-kata  arc-runners   deployed  gha-runner-scale-set-0.14.2             0.14.2
...
# cxn-kata-<hash>-listener                 1/1   Running
```

---

## 3. Scale-set values (`arc-cxn-values.yaml`)
...
This is the live `arc-cxn-values.yaml` verbatim, with only the repo owner/name in
`githubConfigUrl` redacted:

```yaml
githubConfigUrl: "https://github.com/<OWNER>/<REPO>"
githubConfigSecret: arc-github
runnerScaleSetName: cxn-kata
minRunners: 0
maxRunners: 8
...
template:
  spec:
    ...
    containers:
      - name: runner
        image: cxn-kata-runner:2026-07-27-072222
...
1. The **listener** (ns `arc-systems`) long-polls the GitHub Actions service for
   jobs whose `runs-on` matches `cxn-kata`.
...
```bash
kubectl -n arc-runners get autoscalingrunnerset cxn-kata
...
| --- | --- | --- |
| cxn-kata runner pods (trusted `push`/main + release) | `grpcs://bazel-remote.bazel-cache.svc.cluster.local:9092` | yes - `ci` credentials injected via the `bazel-remote-ci` secret |
...
- **Secrets** (all maintained by `setup.sh`):
  ...
  - `arc-runners/bazel-remote-ci` - `BAZEL_REMOTE_USER` / `BAZEL_REMOTE_PASSWORD`,
    ...
    ([step 3](#3-scale-set-values-arc-cxn-valuesyaml); `infra/reload-runner.sh`
    inserts the `envFrom` entry into `arc-cxn-values.yaml` idempotently on the
    next image reload).
...
On cxn-kata the credentials come from the injected pod env
...
the runner-cache PVC (`CXN_XWIN_CACHE_DIR`).
...
wraps `bun install --frozen-lockfile`. On cxn-kata, the pod template mounts
...
- `ci.yml` routes every pull-request job to GitHub-hosted runners
  (`runs-on` resolves to `cxn-kata` only for `push`/main, manual dispatch, and
...
- `bazel-cache/bazel-remote-data` - bazel-remote evicts LRU at `--max_size 90`
  ...
  if hit rates drop.
- `arc-runners/runner-cache` - coarse manual cleanup: scale `cxn-kata` to zero,
...
# CXN_NATIVE_CACHE_DIR from arc-cxn-values.yaml (+ helm upgrade).
```
...
- **No cluster rights.** Jobs run under `cxn-kata-gha-rs-no-permission` with no
...
```bash
kubectl -n arc-runners get autoscalingrunnerset cxn-kata   # min/max/current runners
...
**Verify the caches are being used.** A warm Bazel build on cxn-kata logs
...
**Resize a job's VM** - edit the `resources` block in `arc-cxn-values.yaml`
([step 3](#3-scale-set-values-arc-cxn-valuesyaml); requests = guaranteed VM size,
limits = hotplug ceiling) and roll out:

```bash
helm upgrade cxn-kata \
...
convenience wrapper, `cxn-add-repo-runner <OWNER>/<REPO> [label]`, performs exactly
this install.)

**Uninstall** (leaves k3s/Kata in place):

```bash
helm uninstall cxn-kata -n arc-runners
