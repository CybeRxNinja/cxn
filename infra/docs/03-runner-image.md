...
Build context lives at `/root/cxn-kata-runner-image/`. The Dockerfile below is
...
> the `/root/cxn-kata-runner-image/` copy is overwritten from it by the
> repo-driven reload script (see section 3 below).

```dockerfile
# syntax=docker/dockerfile:1
# Preloaded cxn-kata runner image.
#
...
#
# Rebuild + reimport (see /root/cxn-kata-runner.md) after bumping the ARGs below
...
`/root/cxn-kata-runner-image/reload.sh` does the whole cycle: build, in-image
...
```bash
#!/usr/bin/env bash
# Rebuild the preloaded cxn-kata runner image, import it into k3s containerd,
...
#
#   ./reload.sh              # build tag cxn-kata-runner:YYYY-MM-DD-HHMMSS
#   ./reload.sh 2026-06-20   # build tag cxn-kata-runner:2026-06-20
...
case "$arg" in *:*) IMAGE="$arg";; *) IMAGE="cxn-kata-runner:$arg";; esac

echo "==> [1/5] building $IMAGE"
DOCKER_BUILDKIT=1 docker build -t "$IMAGE" -t cxn-kata-runner:preloaded .
...
sed -i "s#image: cxn-kata-runner:.*#image: $IMAGE#" /root/arc-cxn-values.yaml
helm upgrade cxn-kata --namespace arc-runners --version 0.14.2 \
  -f /root/arc-cxn-values.yaml \
  oci://ghcr.io/actions/actions-runner-controller-charts/gha-runner-scale-set >/dev/null

echo "==> [5/5] verifying rollout"
live="$(kubectl get autoscalingrunnerset cxn-kata -n arc-runners -o jsonpath='{.spec.template.spec.containers[0].image}')"
...
`$1`: no arg gives a timestamped `cxn-kata-runner:YYYY-MM-DD-HHMMSS`; an argument
containing a colon (`foo:bar`) is used as an explicit `repo:tag`; anything else
is treated as a tag suffix on `cxn-kata-runner:`.

**[1/5] build.** `DOCKER_BUILDKIT=1 docker build` tags the result twice: the
immutable `$IMAGE` (dated) and the moving `cxn-kata-runner:preloaded` alias.
...
`image: cxn-kata-runner:...` line in `/root/arc-cxn-values.yaml` to the new tag,
...
- `docker save | k3s ctr -n k8s.io images import` places the image directly into
  ...
  reference `cxn-kata-runner:<tag>` to `docker.io/library/cxn-kata-runner:<tag>`
...
| --- | --- | --- | --- | --- |
| `cxn-kata-runner:YYYY-MM-DD-HHMMSS` | immutable | yes | yes | the build of record; what runners actually boot |
| `cxn-kata-runner:preloaded` | moving | no | no | docker-local alias to the most recent build |
...
- Live example at time of writing: the scale set references
  `cxn-kata-runner:2026-06-15-002621`, held in containerd as
  `docker.io/library/cxn-kata-runner:2026-06-15-002621`.
...
```bash
docker run --rm --entrypoint bash cxn-kata-runner:preloaded -lc '
  set -e
  for b in gh fd rg magick bun cargo rustc pkg-config clang lld sccache zig cargo-nextest cargo-zigbuild cargo-xwin; do
    command -v "$b" >/dev/null || { echo "MISSING: $b"; exit 1; }
  done
...
```bash
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
kubectl get autoscalingrunnerset cxn-kata -n arc-runners \
  -o jsonpath='{.spec.template.spec.containers[0].image}{"\n"}'
k3s ctr -n k8s.io images ls | grep cxn-kata-runner
```
...
kubectl run preload-verify -n arc-runners --restart=Never \
  --image=cxn-kata-runner:2026-06-15-002621 \
