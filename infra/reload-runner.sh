#!/usr/bin/env bash
# Build + roll the preloaded cxn-kata runner image onto the self-hosted CI host,
...
#
# Usage:
#   CI_HOST=my-ci-host ./infra/reload-runner.sh                # tag: cxn-kata-runner:YYYY-MM-DD-HHMMSS
#   CI_HOST=my-ci-host ./infra/reload-runner.sh 2026-06-20     # tag: cxn-kata-runner:2026-06-20
...
#   REMOTE_CTX               remote build dir for the Dockerfile           [/root/cxn-kata-runner-image]
#   ARC_VALUES               remote ARC scale-set helm values file         [/root/arc-cxn-values.yaml]
#   ARC_RELEASE              helm release name of the runner scale set     [cxn-kata]
...
REMOTE_CTX="${REMOTE_CTX:-/root/cxn-kata-runner-image}"
ARC_VALUES="${ARC_VALUES:-/root/arc-cxn-values.yaml}"
ARC_RELEASE="${ARC_RELEASE:-cxn-kata}"
...
build_with_containerd() {
  ...
  k3s ctr -n k8s.io images tag "$IMAGE" cxn-kata-runner:preloaded >/dev/null 2>&1 || true
...
sed -i "s#image: cxn-kata-runner:.*#image: $IMAGE#" "$ARC_VALUES"
