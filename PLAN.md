...
|---|---|---|
| **0. Bootstrap** ✅ | Repo created + published; upstream remotes; rebrand sweep; branding guard; CI green on hosted runners | `cxn` binary builds & runs; zero forbidden-branding matches; CI green |
...
| **5. Automation** 🔶 | **Upstream sync ✅** (weekly + manual, auto-merge when green). **Remaining:** release pipeline on hosted runners (§5), GitHub Packages publishing, installer/self-update, branding guard as required check | A full release (tag → binaries → GitHub Release → npm) completes |
