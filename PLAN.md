...
| **2. First port wave (decided: rlm + daemon + /refine together)** | **`rlm()` + `agent_message` DONE** — `eval/py/rlm.ts` (bridge dispatch in `eval/js/tool-bridge.ts`), prelude helpers `rlm.*`/`agent_message.*` mirroring the upstream callable API, in-memory family registry + mailboxes, spawn via `runStructuredSubagent` (keepAlive + retainArtifacts), limits (name 64, msg 16KiB, 20 pending/mailbox), tests in `test/eval/py/rlm.test.ts` + prelude payload tests (see [docs/rlm.md](docs/rlm.md)). **`/refine` DONE** — `src/refinement/` (types/state/refinement/memory-backend) + `/refine` slash command (local/global/rollback/history), snapshot+rollback via before/after per edit, memory-kind entries synced into the active memory backend, harness overview injected into the system prompt, 33 tests (see [docs/refine.md](docs/refine.md)). Remaining: child kernels wired into family, siblings, `find_models` catalog, compaction-surviving persistence; auto-refine hookup after daemon lane. Then port `modes/daemon/` + `agent-connection/` + session leases (`cxn agents/attach/send/...`). Parallelizable tracks; land behind feature flags | `rlm()` spawns children + messages flow + registry survives compaction; detach → worker survives → reattach with replay; `/refine` applies + rolls back harness updates |
