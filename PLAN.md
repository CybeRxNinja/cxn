...
| **1. RLM spike + kernel** | Spike: RLM-style turn through cxn `eval` kernel (persistent py cell → host request → tool call); extend `eval`/`agent-bridge` to full host-request protocol; `cxn-runtime` pkg; `/ipython`-style surface decision | End-to-end RLM turn works; spike report written; decision recorded |
| **2. First port wave (decided: rlm + daemon + /refine together)** | Port `rlm-runtime.ts`, `rlm-max-depth.ts`, `agent-messages.ts` (child registry, parent↔child messaging); port `modes/daemon/` + `agent-connection/` + session leases (`cxn agents/attach/send/...`); port `core/refinement/` → `/refine` integrated with cxn memory backends. Parallelizable tracks; land behind feature flags | `rlm()` spawns children + messages flow + registry survives compaction; detach → worker survives → reattach with replay; `/refine` applies + rolls back harness updates |
