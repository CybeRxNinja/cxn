...
pub struct MinimizerOptions {
	...
	/// `CXN_MINIMIZER_LEGACY_FILTERS` environment variable (truthy = "1",
...
pub struct MinimizerConfig {
	...
	/// `MinimizerOptions.legacy_filters` or the `CXN_MINIMIZER_LEGACY_FILTERS`
...
impl MinimizerConfig {
	...
	pub fn from_options(opts: &MinimizerOptions) -> Self {
		...
		let legacy_requested = resolve_legacy_filters(
			opts.legacy_filters,
			std::env::var("CXN_MINIMIZER_LEGACY_FILTERS")
				.ok()
				.as_deref(),
		);
...
/// and the raw `CXN_MINIMIZER_LEGACY_FILTERS` env value.
///
...
mod tests {
	...
	fn explicit_disabled_option_overrides_enabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("cxn-minimizer-config-disabled-{}.toml", std::process::id()));
	...
	fn explicit_legacy_true_overrides_disabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("cxn-minimizer-config-legacy-{}.toml", std::process::id()));
