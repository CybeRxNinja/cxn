...
//!   `$XDG_STATE_HOME/cxn/logs/` on Linux / macOS when the user has migrated to
...
/// Default directory name for CXN's per-user state (overridable via
/// `PI_CONFIG_DIR`, matching `packages/utils/src/dirs.ts`).
const DEFAULT_CONFIG_DIR: &str = ".cxn";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/cxn/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "cxn";
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/cxn/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/cxn` exists on disk, and
...
/// reads, no fs reads. `cxn_dir_exists` decides whether the candidate
/// `<xdg_state_home>/cxn` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
	xdg_state_home: Option<&OsStr>,
	agent_dir_override: Option<&OsStr>,
	default_agent_dir: &Path,
	cxn_dir_exists: impl FnOnce(&Path) -> bool,
) -> Option<PathBuf> {
	...
	let cxn_dir = Path::new(xdg).join(APP_NAME);
	if !cxn_dir_exists(&cxn_dir) {
		return None;
	}
	Some(cxn_dir.join("logs"))
}
#[cfg(any(target_os = "linux", target_os = "macos"))]
...
mod tests {
	...
	fn resolve_logs_dir_defaults_under_dot_omp() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.cxn/logs"));
	}
	...
	fn resolve_logs_dir_honors_relative_pi_config_dir() {
		let dir = resolve_logs_dir(
			...
			Some(OsStr::new(".cxn-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.cxn-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		let dir = xdg_state_logs(
			...
			Some(OsStr::new("")),
			Path::new("/tmp/pi-natives-test-home/.cxn/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/cxn/logs")));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/cxn/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/cxn/logs"));
	}
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn xdg_state_logs_skipped_when_cxn_dir_missing() {
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			None,
			Path::new("/tmp/pi-natives-test-home/.cxn/agent"),
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/pi-natives-test-home/.cxn/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.cxn/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.cxn/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/cxn/logs")));
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_cxn_by_default() {
		let dir = default_agent_dir(Path::new("/tmp/pi-natives-test-home"), None);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.cxn/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_respects_pi_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/pi-natives-test-home"), Some(OsStr::new(".cxn-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.cxn-dev/agent"));
	}

	#[test]
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/pi-natives-test-home/.cxn/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/pi-natives-test-home/.cxn/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/pi-natives-test-home/.cxn/logs/native-alloc-99-1.log")
		);
	}
}
