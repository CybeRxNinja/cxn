...
import { CURSOR_MARKER, TUI } from "@cyberxninja-omp/pi-tui";
...
import { KeybindingsManager, setKeybindings, TUI_KEYBINDINGS } from "@cyberxninja-omp/pi-tui/keybindings";
import { setKittyProtocolActive } from "@cyberxninja-omp/pi-tui/keys";
...
describe("Editor component", () => {
	...
	describe("Grapheme-aware text wrapping", () => {
		...
		it("keeps terminal-local IME preedit from displacing the editor border (#5563)", async () => {
			...
			try {
				...
				// committed bytes reach CXN. The open cursor row must not carry right
	...
	describe("Korean NFC paste normalization", () => {
		...
		// landed on the legacy `Input` component; CXN's interactive prompt
