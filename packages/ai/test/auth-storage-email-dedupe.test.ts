...
import { AuthStorage, type FetchImpl, type OAuthCredential, SqliteAuthCredentialStore } from "@cyberxninja-omp/pi-ai";
...
describe("AuthStorage OAuth login upgrade and multi-account coexistence", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-auth-login-test-"));
	});
...
describe("AuthStorage persistent session stickiness", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-auth-test-"));
