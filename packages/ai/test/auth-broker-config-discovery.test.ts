...
import { discoverAuthStorage, resolveAuthBrokerConfig } from "@cyberxninja-omp/pi-ai/auth-broker";
import { removeWithRetries } from "../../utils/src/temp";
import { withEnv } from "./helpers";

const SUPPRESS_AUTH_BROKER_ENV = {
	CXN_AUTH_BROKER_URL: undefined,
	CXN_AUTH_BROKER_TOKEN: undefined,
	CXN_AUTH_BROKER_ACCOUNT_POOL_FILE: undefined,
} as const;
...
describe("resolveAuthBrokerConfig config discovery", () => {
	...
	test("rejects unreadable or malformed account-pool files before connecting", async () => {
		...
		const brokerEnv = {
			...
			CXN_AUTH_BROKER_URL: "http://127.0.0.1:1",
			CXN_AUTH_BROKER_TOKEN: "test-token",
			CXN_AUTH_BROKER_ACCOUNT_POOL_FILE: poolPath,
		} as const;

		await withEnv(brokerEnv, async () => {
			await expect(discoverAuthStorage({ agentDir })).rejects.toThrow(
				"Unable to read CXN_AUTH_BROKER_ACCOUNT_POOL_FILE",
			);
