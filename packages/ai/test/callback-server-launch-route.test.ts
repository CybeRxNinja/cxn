...
import { OAuthCallbackFlow } from "@cyberxninja-omp/pi-ai/registry/oauth/callback-server";
import type { OAuthAuthInfo, OAuthCredentials } from "@cyberxninja-omp/pi-ai/registry/oauth/types";
...
describe("OAuthCallbackFlow /launch route", () => {
	...
	it("suppresses launchUrl and routes /launch to the callback handler when callbackPath is /launch", async () => {
		...
		const flow = new LaunchProbeFlow(
			...
			// Caller pins the provider redirect at `/launch` — an CXN config
