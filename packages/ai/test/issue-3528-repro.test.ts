 ...
 *
 * `cxn-llm-request-15179edfab4dc557.json` plus the rr-session captures from the
 * reporter showed:
 *
 ...
 * thinking output). The CXN-side `Context` preserved those as a
...
import { applyChatCompletionsReasoningParams, type OpenAICompletionsParams } from "@cxn/pi-ai/providers/openai-shared";
import type { AssistantMessage, Message, Model, ModelSpec, ThinkingContent, UserMessage } from "@cxn/pi-ai/types";
...
describe("llama.cpp warm-prefix preservation (#3528)", () => {
	...
	it("leaves qwenPreserveThinking off for cloud Qwen hosts", () => {
		...
		// cloud side; CXN isn't responsible for KV-cache invalidation there,
