...
import { __providerInFlightForTesting, configureProviderMaxInFlightRequests, streamSimple } from "@cxn/pi-ai/stream";
...
async function useIsolatedLimiterRoot(): Promise<void> {
	limiterRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-provider-inflight-test-"));
