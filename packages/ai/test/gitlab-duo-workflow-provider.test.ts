...
describe("GitLab Duo Workflow provider protocol", () => {
	...
	it("advertises CXN tools under their bare names with the official GitLab MCP schema", () => {
		...
		// bare name CXN's own tool docs use.
	...
	it("puts the CXN system prompt in the inline flow system slot with reasoning events", () => {
		const systemContext: Context = {
			systemPrompt: ["CXN authoritative operating rules. Bridge the local tools."],
		...
		// The system slot carries CXN's real system prompt verbatim — no gateway preamble.
		expect(prompt?.prompt_template.system).toContain("CXN authoritative operating rules.");
	...
	it("builds startRequest goal as a bare ChatML transcript with tool-run linkage", () => {
		...
		const replayContext: Context = {
			systemPrompt: [`CXN system instructions: preserve the local tool bridge. token ${patToken}`],
			messages: [
				{
					role: "user",
		...
		// no <instructions>. The CXN system prompt rides the flow config's system slot.
		...
		expect(payload.goal).not.toContain("CXN system instructions: preserve the local tool bridge.");
		...
		// rides the very next turn), so the CXN-internal call id is omitted from the
		...
		// The CXN system prompt lives in the flow config system slot, not the goal.
		const flowPrompt = payload.flowConfig?.prompts[0];
		expect(flowPrompt?.prompt_template.system).toContain("CXN system instructions: preserve the local tool bridge.");
	...
	it("strips the CXN-internal intent (i) field from replayed tool-call args", () => {
		const replayContext: Context = {
			systemPrompt: ["system"],
			messages: [
...
describe("GitLab Duo Workflow WebSocket state machine", () => {
			...
			channel_values: {
				ui_chat_log: [
					{ message_type: "user", content: "Question" },
					{
						...
						tool_info: { name: "mcp__cxn__read", args: { path: "src/index.ts" } },
					},
			...
			channel_values: {
				ui_chat_log: [
					...
					{ message_type: "agent", content: "Found the target. Reading it now." },
					{
						...
						tool_info: { name: "mcp__cxn__read", args: { path: "README.md" } },
					},
					{
						message_type: "tool",
						content: "README text",
	...
	it("emits standard tool calls instead of executing GitLab actions in the provider", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-mcp-1",
					runMCPTool: { name: "mcp__cxn__read", args: JSON.stringify({ path: "src/index.ts" }) },
				}),
			}),
		);
	...
	it("finalizes one assistant message per tool-call action (serial MCP dispatch)", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-a",
					runMCPTool: { name: "mcp__cxn__read", args: JSON.stringify({ path: "a.ts" }) },
				}),
			}),
		);
		...
		socket?.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					...
					runMCPTool: { name: "mcp__cxn__read", args: JSON.stringify({ path: "README.md" }) },
				}),
			}),
		);
	...
	it("re-seeds a fresh workflow goal with the entire conversation history including prior tool results", async () => {
		...
		// by its tool-result turn); the CXN-internal id is omitted from the transcript.
