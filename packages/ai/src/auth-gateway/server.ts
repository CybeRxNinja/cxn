 ...
 * pure protocol translation: foreign wire → cxn Context → pi-ai stream() →
 * cxn events → foreign wire.
 *
 * Endpoints:
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (cxn re-stamps every parsed Message). role +
