...
import { getAgentDbPath, getDbBusyTimeoutMs, logger } from "@cxn/pi-utils";
import type {
	AuthCredential,
	AuthCredentialStore,
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent cxn startups can race against WAL recovery and the schema
	 ...
	 * the connection: concurrent cxn startups race WAL recovery and the leases
