...
import { getInstallId, logger } from "@cxn/pi-utils";
import {
	type AuthCredential,
...
export class RemoteAuthCredentialStore implements AuthCredentialStore {
	...
	replaceAuthCredentialsForProvider(_provider: string, _credentials: AuthCredential[]): StoredAuthCredential[] {
		throw new AIError.AuthBrokerError(
			"RemoteAuthCredentialStore is read-only on the client. Use `cxn auth-broker login <provider>` to mutate credentials.",
		);
	}
	...
	deleteAuthCredentialsForProvider(_provider: string, _disabledCause: string): void {
		throw new AIError.AuthBrokerError(
			"RemoteAuthCredentialStore is read-only on the client. Use `cxn auth-broker logout <provider>` to mutate credentials.",
		);
	}
