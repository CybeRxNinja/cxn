/**
 * OpenAI Responses HTTP wire-format ↔ cxn Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes cxn's {@link AssistantMessage} (and event stream) back into
...
import { logger, structuredCloneJSON } from "@cxn/pi-utils";
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that cxn's gateway doesn't honour;
					...
					switch (ev.type) {
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the cxn side, the
