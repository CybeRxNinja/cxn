...
import { $env, isEnoent, logger } from "@cyberxninja-omp/pi-utils";
...
async function stsAssumeRole(
	...
	const body = new URLSearchParams({
		...
		Version: "2011-06-15",
		RoleArn: roleArn,
		RoleSessionName: opts.sessionName || `cxn-${process.pid}`,
	});
...
async function assumeRoleWithWebIdentity(
	...
	const body = new URLSearchParams({
		...
		RoleSessionName: params.sessionName || `cxn-${process.pid}`,
