...
```bash
npm install @cxn/pi-ai
```

## Quick Start

```typescript
import { getModel, stream, complete, Context, Tool, type } from "@cxn/pi-ai";
...
```typescript
import { type Tool, type } from "@cxn/pi-ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@cxn/pi-ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@cxn/pi-ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@cxn/pi-ai";
...
```typescript
import { getModel, stream } from "@cxn/pi-ai";
...
```typescript
import { getProviders, getModels, getModel } from "@cxn/pi-ai";
...
```typescript
import { Model, stream } from "@cxn/pi-ai";
...
```typescript
import { getModel, complete, Context } from "@cxn/pi-ai";
...
```typescript
import { Context, getModel, complete } from "@cxn/pi-ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so cxn injects
...
```typescript
import { getEnvApiKey } from "@cxn/pi-ai";
...
Authenticate via the [`cxn`](https://cxn.sh) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
cxn auth-broker login              # interactive provider selection
cxn auth-broker login anthropic    # login to a specific provider
cxn auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
cxn auth-broker list               # list supported providers
cxn auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@cxn/pi-ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@cxn/pi-ai";
import * as fs from "node:fs";
