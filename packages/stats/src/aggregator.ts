...
import { getStatsDbPath, workerHostEntry } from "@cxn/pi-utils";
...
// JavaScript entry. Standalone source `cxn-stats` keeps using this package's
...
function defaultWorkerCount(): number {
	...
	// the compiled `cxn` binary. Keep macOS on the documented serial path.
 ...
 * self-dispatching CLI entry (cxn in source, npm-bundle, or compiled form),
 * re-enter that entry with a worker argv selector; otherwise (standalone
 * cxn-stats, bun test, SDK embedding) load the worker module directly, so this
 * package keeps zero runtime dependency on `@cxn/pi-coding-agent`.
 */
function createSyncWorker(): Worker {
	const hostEntry = workerHostEntry();
	if (hostEntry) {
		return new Worker(hostEntry, { type: "module", argv: ["__cxn_worker_stats_sync"] });
	}
 ...
 * then terminates. Used by `cxn --smoke-test` so the install-method CI jobs
