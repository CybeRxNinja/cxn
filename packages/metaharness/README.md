...
1. **Local cxn, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs cxn
   ...
   `/opt/cxn/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/cxn-linux-*` self-contained binaries).
...
| `--env <KEY[=VALUE]>` | — | Forward env into the cxn container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt cxn binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@cxn/pi-coding-agent` |
...
arc; ~$0.001 per report). API keys resolve through cxn's auth storage.
