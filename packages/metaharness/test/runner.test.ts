...
describe("generic agent-arg / env passthrough", () => {
	it("forwards repeated --agent-arg as a JSON array the in-container agent can parse", () => {
		...
		expect(JSON.parse(env.CXN_BENCH_AGENT_ARGS ?? "[]")).toEqual(cfg.agentArgs);
	});

	it("omits CXN_BENCH_AGENT_ARGS when no --agent-arg was passed", () => {
		const cfg = parseArgs(["--model", "anthropic/claude-opus-4-8"]);
		const env = buildHarborEnv(cfg, "/tmp/models.yml", null, "test");
		expect(env.CXN_BENCH_AGENT_ARGS).toBeUndefined();
	});
	...
	it("explicit --providers is authoritative; the default derives from the model", () => {
		...
		expect(new Set(envExplicit.CXN_BENCH_GATEWAY_PROVIDERS?.split(","))).toEqual(new Set(["google"]));
		...
		expect(new Set(envDerived.CXN_BENCH_GATEWAY_PROVIDERS?.split(","))).toEqual(new Set(["anthropic"]));
	});
