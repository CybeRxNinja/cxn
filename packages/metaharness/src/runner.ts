...
/**
 * Resolver injected into Apple Container runs (CXN_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.CXN_BENCH_CONTAINER_DNS || "1.1.1.1";
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "cxn-compose-overlay.yaml");
...
export function buildHarborEnv(
	...
	env.CXN_BENCH_INSTALL = cfg.install;
	env.CXN_BENCH_VERSION = cfg.version ?? version;
	if (tarball) env.CXN_BENCH_TARBALL = tarball;
	if (source) {
		env.CXN_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.CXN_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.CXN_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.CXN_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	if (cfg.binaryX64) env.CXN_BENCH_BINARY_X64 = cfg.binaryX64;
	if (cfg.thinking) env.CXN_BENCH_THINKING = cfg.thinking;
	if (cfg.agentArgs.length > 0) env.CXN_BENCH_AGENT_ARGS = JSON.stringify(cfg.agentArgs);
	if (cfg.webSearch) env.CXN_BENCH_WEB_SEARCH = "1";
	env.CXN_BENCH_GATEWAY = cfg.gateway ? "1" : "0";
	if (cfg.gateway) {
		env.CXN_BENCH_MODELS_YAML = modelsYaml;
		env.CXN_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.CXN_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.CXN_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.CXN_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.CXN_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "omp" && cfg.gateway) {
		...
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'cxn-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		if (harborEnv.CXN_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.CXN_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  CXN_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
