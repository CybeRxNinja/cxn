 ...
 *
 * Auth: provider API keys resolve through cxn's auth storage
 * (~/.cxn/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@cxn/pi-ai";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`cxn login\` or set the provider env var)`);
	}
