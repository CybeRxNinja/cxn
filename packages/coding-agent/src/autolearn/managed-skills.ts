 ...
 * isolated directory (`~/.cxn/agent/managed-skills`) separate from
 * user-authored skills (`~/.cxn/agent/skills`). They are discovered and
...
import { getAgentDir, isEnoent } from "@cxn/pi-utils";
import { YAML } from "bun";

/** Provider id stamped on discovered managed skills (distinguishes them from authored). */
export const MANAGED_SKILLS_PROVIDER_ID = "cxn-managed";
...
/** Resolve the isolated managed-skills directory (`~/.cxn/agent/managed-skills`). */
