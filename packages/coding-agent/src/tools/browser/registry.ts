...
export interface PuppeteerBrowserHandle extends BrowserHandleCommon {
	...
	cdpUrl?: string;
	pid?: number;
	/** CXN-owned temp Chromium profile directory removed on dispose (process-local headless launches). */
	userDataDir?: string;
...
async function disposeBrowserHandle(handle: BrowserHandle, opts: ReleaseBrowserOptions): Promise<void> {
	...
	if (handle.kind.kind === "headless") {
		...
		// CXN owns the profile directory (puppeteer's temp cleanup is disabled by
