 ...
 *
 * Puppeteer clients (the cxn browser tool: one supervisor connection plus one
...
class CdpConnection {
	...
	/** Tabs this connection claimed as drive targets (`CXN.claimTarget` / `Target.createTarget`). */
...
class TabState {
	...
	/** True after the relay put this tab in the cxn group; `ompGroupId` holds that group. */
	grouped = false;
	/** Group RPC in flight — suppresses duplicate requests from load-time tabUpdated bursts. */
	grouping = false;
	ompGroupId: number | undefined;
	/** User pulled the tab out of the cxn group — never re-group it. */
	groupOptOut = false;
...
export class RelayBridge {
	...
	extClosed(socket: RelaySocket): void {
		...
		for (const tab of this.#tabs.values()) {
			tab.attached = false;
			tab.attaching = null;
			// The extension dissolves cxn groups on disconnect (or died along
	...
	cdpClosed(connId: number): void {
		...
		conn.sessions.clear();
		// Tabs this client claimed leave the cxn group unless another claimant
	...
	async #forwardToTab(
		...
		// Relay-private claim: the cxn tab worker marks the page it was spawned
		// to drive. Never forwarded — real Chrome rejects the unknown method.
		if (msg.method === "CXN.claimTarget") {
	...
	async #handleBrowserCommand(conn: CdpConnection, msg: CdpCommand): Promise<void> {
		switch (msg.method) {
			...
			case "Target.createBrowserContext":
				this.#replyError(conn, msg, "Browser contexts are not supported by the cxn browser relay");
				return;
			default:
	...
	#onTabDetached(tabId: number, reason: string): void {
		...
		// down): release the tab's cxn-group membership too.
		this.#syncTabGrouping(tab);
		this.#retractTab(tab);
	}
	...
	#onTabUpsert(snap: TabSnapshot, opts: { silent?: boolean } = {}): void {
		...
		if (!tab) {
			...
			// The user dragging a tab out of the cxn group is an opt-out; the
	...
	/** A tab belongs in the cxn group when claimed by a client, controllable, unpinned, not user-opted-out, and not already in a user group. */
	 ...
	 * duplicate cxn groups, so at most one group RPC is ever in flight.
	 */
