...
export interface LaunchHeadlessResult {
	browser: Browser;
	/**
	 * CXN-owned temporary Chromium profile directory to remove after the browser
	 * process tree exits, or `undefined` when the caller supplied its own
	 * `--user-data-dir` (which CXN must not delete).
	 */
	userDataDir?: string;
}
...
export async function launchHeadlessBrowser(opts: LaunchHeadlessOptions): Promise<LaunchHeadlessResult> {
	...
	if (!launchArgs.some(arg => arg.startsWith("--user-data-dir"))) {
		userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cxn-chrome-profile-"));
...
/**
 * Remove an CXN-owned headless Chromium profile directory, tolerating the brief
