...
import { getProjectDir, readJsonl } from "@cyberxninja-omp/pi-utils";
...
export interface StdioSpawnCommand {
	 ...
	 *
	 * Windows uses this only when the CXN host has no console to share. When
	 ...
	 * wrapper chains must stay in the CXN console session so nested console
...
export async function resolveStdioSpawnCommand(
	...
	// same console session. Only hide the child when CXN itself has no console
