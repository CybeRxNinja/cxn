...
/**
 * Deterministic credential id for an MCP server URL scoped to an CXN profile.
 *
...
/** Whether a credential id was minted by CXN's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for CXN-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not CXN's server-URL fallback. Preserve same-host
