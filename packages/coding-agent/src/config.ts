...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@cxn/pi-utils";
 ...
 * project with cxn's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as cxn's startup changelog).
 */
...
/**
 * Path to cxn's own `CHANGELOG.md`, or `undefined` when the package directory
...
/**
 * User-level: ~/.cxn/agent, Claude's active config directory, ~/.codex, ~/.gemini
 * Project-level: .cxn, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".cxn", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.cxn/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.cxn/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.cxn/agent/commands", source: ".cxn", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.cxn, .claude) - the nearest one found.
