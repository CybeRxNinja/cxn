...
export const CXN_SECURITY_WORKFLOW_VERSION = "1.0.0";

export function createNativeSecurityProducer(): SecurityProducer {
	return {
		kind: "cxn-native",
		name: "CXN Native Security",
		version: CXN_SECURITY_WORKFLOW_VERSION,
	};
}

export function createSecurityCredentialAffinity(account: SecurityAccountRef): string {
	return `cxn-security-credential/v1:sha256:${Bun.SHA256.hash(canonicalSecurityJson(account), "hex")}`;
}
...
export function createNativeSecurityProvenance(options: {
	...
	if (options.sessionId !== undefined) {
		metadata.sessionAffinity = `cxn-security-session/v1:sha256:${Bun.SHA256.hash(options.sessionId, "hex")}`;
	}
...
export function createSecurityWorkflowFingerprint(inputs: readonly string[]): string {
	return `cxn-security-workflow/v1:sha256:${Bun.SHA256.hash(
		canonicalSecurityJson({
			workflowVersion: CXN_SECURITY_WORKFLOW_VERSION,
			upstream: CODEX_SECURITY_UPSTREAM,
			inputs,
		}),
		"hex",
