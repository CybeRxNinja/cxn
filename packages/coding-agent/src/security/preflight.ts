...
async function digestWorkingTree(
	...
	hasher.update(head);
	return `cxn-security-tree/v1:sha256:${hasher.digest("hex")}`;
}
...
async function normalizeTarget(
	...
	if (request.kind === "ref_diff") {
		...
		return {
			...
			includePaths,
			excludePaths,
			treeDigest: `cxn-security-diff/v1:sha256:${Bun.SHA256.hash(
...
async function buildPlanMaterial(
	...
	return {
		...
		model,
		account,
		configFingerprint: `cxn-security-config/v1:sha256:${Bun.SHA256.hash(canonicalSecurityJson(request.config), "hex")}`,
...
export async function createSecurityScanPlan(
	...
	const fingerprint = `cxn-security-plan/v1:sha256:${Bun.SHA256.hash(canonicalSecurityJson(material), "hex")}`;
	return parseSecurityScanPlan({
		documentType: "cxn-security.scan-plan",
		schemaVersion: "1.0",
