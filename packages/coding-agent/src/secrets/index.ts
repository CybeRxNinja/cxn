...
import { SENSITIVE_TOKEN_RE } from "@cxn/pi-ai/providers/transform-messages";
import { getSecretPlaceholderKeyPath, isEnoent, logger } from "@cxn/pi-utils";
 ...
 * — `$XDG_STATE_HOME/cxn/secret-placeholder.key` (or `~/.cxn/agent/secret-placeholder.key`
...
export async function loadSecrets(cwd: string, agentDir: string): Promise<SecretEntry[]> {
	const projectPath = path.join(cwd, ".cxn", "secrets.yml");
