/**
 * `cxn compress` — rewrite text files into the dense prompt register.
 *
...
import { getProjectDir, prompt, sanitizeText } from "@cxn/pi-utils";
...
/** User-facing options for `cxn compress`. */
...
export async function resolveCompressTargets(patterns: readonly string[], cwd: string): Promise<string[]> {
	...
	for (const pattern of patterns) {
		if (/[*?[\]{}]/.test(pattern)) {
			// `dot: true` — prompt corpora live under dot directories such as `.cxn/commands`.
 ...
 * stdout for a single-file run (`cxn compress f.md > out.md`).
 */
