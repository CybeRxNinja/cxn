...
// surfaces through `cxn-legacy-pi-bundled:<key>` synthetic modules.
...
const BUNDLED_VIRTUAL_SCHEME = "cxn-legacy-pi-bundled:";
const BUNDLED_VIRTUAL_NAMESPACE = "cxn-legacy-pi-bundled";
...
function synthesizeBundledModuleSourceFromModules(moduleKey: string, modules: BundledModules): string {
	...
	for (const exportName in mod) {
		...
		lines.push(`export const ${exportName} = __cxn_bundled[${JSON.stringify(exportName)}];`);
	}
	if (hasDefault) {
		lines.push("export default __cxn_bundled.default;");
	}
	lines.push("");
	return lines.join("\n");
}
 ...
 * `cxn-legacy-pi-bundled:<key>` import.
 */
...
// is served through the `cxn-legacy-pi-bundled:` virtual namespace (see the
 ...
 * `cxn-legacy-pi-bundled:` virtual namespace (issue #3423). Dev, source-link,
 ...
 * through to the canonical-resolution path. Virtual `cxn-legacy-pi-bundled:`
...
function toImportSpecifier(resolvedPath: string): string {
	// Virtual `cxn-legacy-pi-bundled:` specifiers are served by the synthetic
...
/**
 * Rewrite the extension-owned specifiers CXN must host-resolve — legacy
 ...
 * realpath of every reachable source module CXN must rewrite at load time.
...
/**
 * Linkedom's canvas bridge uses its bundled fallback because CXN does not ship
 * native canvas.
 */
...
export function installLegacyPiSpecifierShim(): void {
	...
	Bun.plugin({
		...
		setup(build) {
			...
			// Compiled mode serves `cxn-legacy-pi-bundled:<key>` imports from
