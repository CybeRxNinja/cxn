...
import { isEnoent, logger } from "@cyberxninja-omp/pi-utils";
...
/**
 * Catalog paths tried in priority order: cxn-namespaced override first, then
 * the Claude Code-compatible fallback so existing marketplaces keep loading.
 */
const CATALOG_RELATIVE_PATHS: readonly string[] = [".cxn-plugin/marketplace.json", ".claude-plugin/marketplace.json"];
