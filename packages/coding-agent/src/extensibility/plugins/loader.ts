...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@cxn/pi-utils";
 ...
 *
 * `home` controls which `<plugins>/cxn-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .cxn and .pi directories).
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/cxn-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "cxn-plugins.lock.json");
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; cxn?: PluginManifest; pi?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.cxn || pluginPkg.pi;
		if (!manifest) {
			// Not an cxn plugin, skip
			continue;
		}
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.cxn/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.cxn/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `cxn-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `cxn`/`pi` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `cxn`/`pi`
...
function readDeclaredManifestEntries(dir: string): DeclaredManifestEntries {
	...
	let pkg: { cxn?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	try {
		pkg = JSON.parse(raw) as { cxn?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	} catch {
		return { declared: false, files: [] };
	}
	const declared = (pkg.cxn ?? pkg.pi)?.extensions;
 ...
 *   1. the directory's own package.json `cxn`/`pi` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `cxn`/`pi`
 ...
 *     and CXN's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `cxn`/`pi` `extensions` manifest are
