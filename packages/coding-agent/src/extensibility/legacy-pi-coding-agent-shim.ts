...
export function createEditToolDefinition(cwd: string, options?: EditToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy EditToolOptions.operations is not supported: CXN's built-in edit tool writes the local " +
...
export function createWriteToolDefinition(cwd: string, options?: WriteToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy WriteToolOptions.operations is not supported: CXN's built-in write tool writes the local " +
...
/**
 * Enumerates the extensions CXN would load through the historical package
...
export class DefaultPackageManager {
	...
	/** Resolve enabled extension paths with their CXN plugin provenance. */
 ...
 *
 * CXN does the same discovery inline inside `createAgentSession()`, so this
 ...
 * override below translates them into CXN's native session options
 ...
 * extensions in the wild — themes are silently dropped (CXN has no
...
/**
 * Legacy pi extensions call `createAgentSession({ resourceLoader })`. CXN's
 ...
 * `noExtensions`/`noSkills` opt-outs), re-run CXN's own discovery, and
 ...
 *
 * Translate the loader's captured state into CXN's option fields, then
 ...
 *
 * Modern CXN auth storage is asynchronous, while older provider extensions
