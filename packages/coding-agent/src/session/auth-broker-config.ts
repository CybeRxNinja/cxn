 ...
 * that flip `CXN_AUTH_BROKER_*` between cases still observe the change, while
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.CXN_AUTH_BROKER_URL ?? ""}\u0000${process.env.CXN_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
