/**
 * Builtin Provider (.cxn)
 *
 * Primary provider for CXN native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@cxn/pi-utils";
...
const DISPLAY_NAME = "CXN";
const DESCRIPTION = "Native CXN configuration from ~/.cxn and .cxn/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.cxn/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .cxn/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.cxn/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.cxn/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://cxn.sh/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.cxn/agent/RULES.md
	// Project scope: nearest .cxn/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .cxn/ directories",
	priority: PRIORITY,
