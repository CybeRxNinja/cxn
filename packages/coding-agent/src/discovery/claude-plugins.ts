 ...
 *
 * `.cxn-plugin/plugin.json` takes precedence over `.claude-plugin/plugin.json`.
...
async function resolvePluginMCPConfig(root: ClaudePluginRoot): Promise<ResolvedMCPConfig> {
	...
	for (const manifestDir of [".cxn-plugin", ".claude-plugin"]) {
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	for (const root of roots) {
		...
		if (resolved.inlineServers) {
			...
			//   nested: { "mcpServers": { name: cfg, ... } }   (CXN/Claude Code project shape)
