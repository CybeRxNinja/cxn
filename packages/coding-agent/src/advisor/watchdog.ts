...
import { getAgentDir, isEnoent, logger, prompt } from "@cxn/pi-utils";
 ...
 * `.cxn/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.cxn/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .cxn/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, ".cxn", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const ownerDir = baseName === ".cxn" ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || baseName === ".cxn") {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .cxn folder, and user agent dir.
