 ...
 *
 * cxn's agent core (`@cxn/pi-agent-core`) emits OpenTelemetry GenAI
 ...
 * vars are set so `cxn` can be observed by any OTLP collector without vendor
 * coupling.
 *
...
import { logger, postmortem } from "@cxn/pi-utils";
import {
	type Attributes,
	type AttributeValue,
	type Counter,
...
/**
 * Periodic flush interval. A long-lived `cxn` process (the ACP server is
...
async function registerProviders(signalConfig: SignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("@cxn/pi-coding-agent"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("@cxn/pi-coding-agent");
...
class AgentMetricRecorder {
	...
	constructor(meter: Meter) {
		...
		this.#chatCostUsd = meter.createCounter("pi.cxn.agent.chat.cost.estimated_usd", {
			description: "Estimated USD cost for completed chat calls.",
			unit: "USD",
		});
		this.#runs = meter.createCounter("pi.cxn.agent.runs", {
			description: "Completed agent runs.",
			unit: "{run}",
		});
		this.#steps = meter.createCounter("pi.cxn.agent.steps", {
			description: "Agent loop steps completed inside a run.",
			unit: "{step}",
		});
		this.#chatCalls = meter.createCounter("pi.cxn.agent.chat.calls", {
			description: "Chat calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#chatDurationMs = meter.createHistogram("pi.cxn.agent.chat.duration", {
			description: "Total chat latency observed in an agent run.",
			unit: "ms",
		});
		this.#toolCalls = meter.createCounter("pi.cxn.agent.tool.calls", {
			description: "Tool calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#toolDurationMs = meter.createHistogram("pi.cxn.agent.tool.duration", {
			description: "Total tool latency observed in an agent run.",
			unit: "ms",
		});
		this.#errors = meter.createCounter("pi.cxn.agent.errors", {
	...
	recordRun(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
		const runAttrs = metricAttributes({
			"pi.cxn.agent.models_used.count": coverage.modelsUsed.length,
			"pi.cxn.agent.providers_used.count": coverage.providersUsed.length,
			"pi.cxn.agent.tools_available.count": coverage.toolsAvailable.length,
			"pi.cxn.agent.tools_invoked.count": coverage.toolsInvoked.length,
			"pi.cxn.agent.tools_unused.count": coverage.toolsUnused.length,
		});
		...
		for (const toolName in summary.tools.byName) {
			...
			for (const status of TOOL_STATUSES) {
				...
				if (count > 0) this.#toolCalls.add(count, metricAttributes({ ...toolAttrs, "pi.cxn.tool.status": status }));
			}
		}
...
function emitRunSummaryLog(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
	emitOtelLog(
		...
		"agent run completed",
		{
			"pi.cxn.agent.step_count": summary.stepCount,
			"pi.cxn.agent.chats.total": summary.chats.total,
			"pi.cxn.agent.chats.total_latency_ms": summary.chats.totalLatencyMs,
			"pi.cxn.agent.tools.total": summary.tools.total,
			"pi.cxn.agent.tools.ok": summary.tools.ok,
			"pi.cxn.agent.tools.error": summary.tools.error,
			"pi.cxn.agent.tools.skipped": summary.tools.skipped,
			"pi.cxn.agent.tools.blocked": summary.tools.blocked,
			"pi.cxn.agent.tools.timeout": summary.tools.timeout,
			"pi.cxn.agent.tools.aborted": summary.tools.aborted,
			"pi.cxn.agent.tools.total_latency_ms": summary.tools.totalLatencyMs,
			"pi.cxn.agent.usage.input_tokens": summary.usage.inputTokens,
			"pi.cxn.agent.usage.output_tokens": summary.usage.outputTokens,
			"pi.cxn.agent.usage.cached_input_tokens": summary.usage.cachedInputTokens,
			"pi.cxn.agent.usage.cache_write_tokens": summary.usage.cacheWriteTokens,
			"pi.cxn.agent.usage.reasoning_output_tokens": summary.usage.reasoningOutputTokens,
			"pi.cxn.agent.usage.total_tokens": summary.usage.totalTokens,
			"pi.cxn.agent.cost.estimated_usd": summary.cost.estimatedUsd,
			"pi.cxn.agent.cost.unavailable_reasons": summary.cost.unavailableReasons.join(","),
			"pi.cxn.agent.errors.total": summary.errors.total,
			"pi.cxn.agent.coverage.tools_available": coverage.toolsAvailable.join(","),
			"pi.cxn.agent.coverage.tools_invoked": coverage.toolsInvoked.join(","),
			"pi.cxn.agent.coverage.tools_unused": coverage.toolsUnused.join(","),
			"pi.cxn.agent.coverage.models_used": coverage.modelsUsed.join(","),
			"pi.cxn.agent.coverage.providers_used": coverage.providersUsed.join(","),
		},
		"pi.cxn.agent.run.completed",
	);
}
...
function emitTelemetryWarningLog(warning: AgentTelemetryWarning): void {
	...
	emitOtelLog("warn", warning.message, attrs, "pi.cxn.telemetry.warning");
}
