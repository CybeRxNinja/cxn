import type { Model, ProviderResponseMetadata, RawSseEvent } from "@cyberxninja-omp/pi-ai";

const MAX_RAW_SSE_EVENTS = 1_000;
const MAX_RAW_SSE_CHARS = 512_000;
const MAX_RAW_SSE_EVENT_CHARS = 64_000;
// Reserve room for the `: cxn-debug-truncated` / `: cxn-debug-elided` marker
...
// (usage/finish_reason) both stay visible. A `: cxn-debug-elided` comment
...
function headTailTrim(lines: string[], budget: number, elidedTotal: number): string[] {
	...
	if (i <= j) {
		...
		out.push(`: cxn-debug-elided chars=${Math.max(0, elided)}`);
		if (tailSlice.length > 0) out.push(`…${tailSlice}`);
	} else if (elided > 0) {
		out.push(`: cxn-debug-elided chars=${elided}`);
	}
	out.push(...tail);
	return out;
}
...
// Any trimmed result ends with the `: cxn-debug-truncated` marker carrying
// the original size.
function trimRawLines(raw: string[]): TrimResult {
	...
	lines.push(`: cxn-debug-truncated originalChars=${originalChars}`);
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: cxn-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
