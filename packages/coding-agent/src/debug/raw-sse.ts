import { type Component, matchesKey, parseSgrMouse, replaceTabs, ScrollView, truncateToWidth } from "@cxn/pi-tui";
	...
	#renderRawLines(innerWidth: number): string[] {
		...
		if (snapshot.droppedRecords > 0) {
			lines.push(
				theme.fg(
					"warning",
					`: cxn-debug-dropped records=${snapshot.droppedRecords} chars=${snapshot.droppedChars}`,
				),
			);
			lines.push("");
		}
		...
		for (const record of snapshot.records) {
			...
			if (record.kind === "event" && record.truncated) {
				lines.push(theme.fg("warning", `: cxn-debug-event-truncated originalChars=${record.originalChars}`));
			}
			lines.push("");
		}
