...
import { isInsideTmux, wrapTmuxPassthrough } from "@cxn/pi-tui/terminal-capabilities";
import { VERSION } from "@cxn/pi-utils/dirs";
	...
	return {
		emit(event): void {
			...
			const body = {
				...
				// Warp resolves this via CLIAgent.command_prefix(); OhMyPi is "cxn".
				agent: "cxn",
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `cxn wants to run ${event.toolName}`,
			});
		});
