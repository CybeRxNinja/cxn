 ...
 */
import { getProjectDir, getSSHConfigPath } from "@cxn/pi-utils";
	...
	async #handleList(): Promise<void> {
		try {
			...
			if (userHosts.length > 0) {
				lines.push(theme.fg("accent", "User level") + theme.fg("muted", ` (~/.cxn/agent/ssh.json):`));
			...
			if (projectHosts.length > 0) {
				lines.push(theme.fg("accent", "Project level") + theme.fg("muted", ` (.cxn/ssh.json):`));
