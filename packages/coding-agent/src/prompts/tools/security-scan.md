CXN-native repository security scans: plan, start, inspect, cancel, validate.
`preflight`: immutable plan pinned to repository snapshot, model, exact OAuth credential.
`start`: plan → background CXN job.
...
`cloud_pull`: cloud findings → canonical CXN security store, available through `security://`.
