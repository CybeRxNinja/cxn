/**
 * Harness documentation index for the `cxn://` protocol.
 *
 ...
 * written by `gen:bundle`) — so `@cxn/pi-coding-agent/*` SDK consumers
...
import { isEnoent, logger } from "@cxn/pi-utils";
 ...
 * SDK consumers importing `@cxn/pi-coding-agent/*` load TypeScript source, where
...
/** Empty index for when no docs corpus is reachable — degrades `cxn://` instead of throwing ENOENT at callers. */
function emptyIndex(): DocsIndex {
	logger.warn(
		"cxn:// docs corpus unavailable: no build-time embed, on-disk docs/ directory, or shipped dist embed found",
	);
