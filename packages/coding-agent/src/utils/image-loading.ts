 ...
 * to PNG/JPEG instead — the automatic equivalent of `CXN_NO_WEBP=1`.
 */
 ...
 * `CXN_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `CXN_NO_WEBP`. */
	excludeWebP?: boolean;
}
