...
#
# `__cxn_emit_referenced_exports` reads captured function bodies from stdin,
...
__cxn_sq_quote() {
	...
	__cxn_qout=
	__cxn_sq=\'
	while case "$__cxn_qbuf" in *$__cxn_sq*) true ;; *) false ;; esac; do
		__cxn_qout=$__cxn_qout${__cxn_qbuf%%"$__cxn_sq"*}"'\\''"
		__cxn_qbuf=${__cxn_qbuf#*"$__cxn_sq"}
	done
	__cxn_qout=$__cxn_qout$__cxn_qbuf
	printf "'%s'" "$__cxn_qout"
}
...
__cxn_emit_export_for() {
	...
	__cxn_xv=
	eval "__cxn_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__cxn_sq_quote "$__cxn_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__cxn_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __cxn_name; do
			__cxn_emit_export_for "$__cxn_name"
		done
}
