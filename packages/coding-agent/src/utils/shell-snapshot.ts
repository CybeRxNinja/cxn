...
echo "# Shell snapshot - generated by cxn agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__cxn_funcs" | __cxn_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __cxn_sq_quote __cxn_emit_export_for __cxn_emit_referenced_exports 2>/dev/null
unset __cxn_funcs __cxn_qbuf __cxn_qout __cxn_sq __cxn_xv __cxn_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), uid === undefined ? "cxn-shell-snapshots" : `cxn-shell-snapshots-${uid}`);
