...
import { getLastChangelogVersionPath, isEnoent, logger } from "@cxn/pi-utils";
...
export function formatStartupChangelogSummary(selection: StartupChangelogSelection): string {
	...
	if (!latestVersion || selection.selectedEntries === 0) {
		return "Updated cxn. Use /changelog for recent changes.";
	}
...
/**
 * Parse changelog entries from cxn's package asset when available, falling back
...
/**
 * Parse an cxn changelog marker version into comparable parts.
 */
...
/**
 * Last cxn version whose changelog the user has seen. Stored as a plain-text
 * marker file (`~/.cxn/agent/last-changelog-version`) rather than in
