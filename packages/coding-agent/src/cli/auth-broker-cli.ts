	...
	try {
		...
		await storage.login(provider, {
			onAuth({ url, launchUrl, instructions }) {
				...
				// sessions where a `launchUrl` (loopback `/launch` on the CXN
				...
				if (launchUrl && launchUrl !== url) {
					// Local shortcut for the machine running CXN. Terminals or
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"CXN_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set CXN_AUTH_BROKER_URL to enable).";
