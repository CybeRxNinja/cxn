/**
 * CLI handler for `cxn worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.cxn/wt/`:
 *
 ...
 *     marker naming the live cxn process; a
...
import { getWorktreesDir, isEnoent } from "@cxn/pi-utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.cxn/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `cxn worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.cxn/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
