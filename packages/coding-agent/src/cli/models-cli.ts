/**
 * `cxn models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.cxn/models.db`
 ...
 */
import type { Api, Effort, Model } from "@cxn/pi-ai";
 ...
 * name doubles as an `cxn models <provider>` shortcut.
 */
...
/** `cxn models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `cxn models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`cxn models find` requires a search substring, e.g. `cxn models find minimax`\n");
		process.exitCode = 1;
		return;
	}
