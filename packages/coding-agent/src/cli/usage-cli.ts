 ...
 *
 * Handles `cxn usage` — fetches provider usage reports for every
...
import { formatDuration, formatNumber, sanitizeText } from "@cxn/pi-utils";
...
/**
 * Tombstones worth a row in `cxn usage`: OAuth credentials torn down
 ...
 * `cxn usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, cxn usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					...
					: `No credentials found${scope}. Run \`cxn\` and use /login to add accounts.\n`;
