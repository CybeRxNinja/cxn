...
import { normalizeProfileName } from "@cxn/pi-utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "cxn") {
		throw new Error('Invalid alias "cxn". Refusing to shadow the base cxn command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "cxn-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> cxn profile alias: ${aliasName} >>>`;
	const end = `# <<< cxn profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps cxn --description 'CXN profile ${profile}'`,
