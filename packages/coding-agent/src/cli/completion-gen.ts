 ...
 */
import type { ArgDescriptor, CliConfig, CommandMetadata, FlagDescriptor } from "@cyberxninja-omp/pi-utils/cli";
...
function bashValueBranch(bin: string, v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return `_cxn_comma "${bashWords(v.values)}"; return 0`;
		case "models":
			return v.multiple
				? `_cxn_comma "$(command ${bin} __complete models 2>/dev/null | cut -f1)"; return 0`
	...
	for (const c of spec.commands) {
		...
		const elseBranch = argWords
			...
			: fileArg
				...
				: ":";
		parts.push(`_cxn_cmd_${bashFn(c.name)}() {
	case "$prev" in
${bashFlagCase(bin, c.flags)}
	...
	for (const c of spec.commands) {
		dispatch.push(`\t\t${commandTokens(c).join("|")})\n\t\t\t_cxn_cmd_${bashFn(c.name)}\n\t\t\t;;`);
	}
	parts.push(`_omp() {
	local cur prev cmd i
	cur="\${CCXN_WORDS[CCXN_CWORD]}"
	prev="\${CCXN_WORDS[CCXN_CWORD-1]}"
	cmd=""
	for (( i=1; i < CCXN_CWORD; i++ )); do
		case "\${CCXN_WORDS[i]}" in
			-*) ;;
			*) cmd="\${CCXN_WORDS[i]}"; break ;;
		esac
	done
	case "$cmd" in
${dispatch.join("\n")}
...
function zshAction(v: ValueSource): string {
	switch (v.kind) {
		...
		case "list":
			return ":value:_cxn_tools";
		case "models":
			return v.multiple ? ":models:_cxn_models_list" : ":model:_cxn_call models";
		case "sessions":
			return ":session:_cxn_call sessions";
		case "file":
			return ":file:_files";
		case "dir":
			return ":dir:_files -/";
...
function generateZsh(spec: CompletionSpec): string {
	const { bin } = spec;
	// The `:value:_cxn_tools` action references this helper; bake its candidates
...
${cmdRows}
	...
	const aliasArms = spec.commands
		.map(c => `\t\t\t${commandTokens(c).join("|")}) _cxn_cmd_${bashFn(c.name)} ;;`)
		.join("\n");
	const rootSpecs = [
...
function generateFish(spec: CompletionSpec): string {
	...
	lines.push(`function __fish_cxn_no_subcommand`);
	...
	const rootCond = "__fish_cxn_no_subcommand";
