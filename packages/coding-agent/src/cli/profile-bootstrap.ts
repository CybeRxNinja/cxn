 ...
 * `@cxn/pi-utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `cxn --system-prompt --profile
 ...
 * `cxn --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`cxn --print --profile work`).
 */
 ...
 * positionals are never stolen (`cxn grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `cxn launch --profile work` and
 * `cxn acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `cxn --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`cxn --some-ext-bool --profile work`
		...
		// activates (`cxn --print --profile work`).
