    (function() {
      ...
      // ============================================================
      //
      // Served by the cxn relay at /s/<id>; the AES-256-GCM key rides in the
      // URL fragment and never leaves the browser. Resolves the session JSON
      // and hands it to template.js via `window.__CXN_SESSION_DATA__`:
      ...
      async function load() {
        ...
        if (data && data.header && data.header.title) {
          document.title = data.header.title + ' — cxn session';
        }
        return data;
      }
      ...
      window.__CXN_SESSION_DATA__ = pending;
    })();
