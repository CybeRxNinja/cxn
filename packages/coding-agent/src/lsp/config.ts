...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@cxn/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.cxn/, .pi/, .claude/)
	...
	// User config directories (~/.cxn/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .cxn/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.cxn/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
