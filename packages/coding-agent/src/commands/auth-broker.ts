/**
 * `cxn auth-broker` — manage the cxn credential vault.
 */
...
export default class AuthBroker extends Command {
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  cxn auth-broker serve",
		"# Boot on a non-default port\n  cxn auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  cxn auth-broker token",
		"# Rotate the bearer token\n  cxn auth-broker token --regenerate",
		"# List supported OAuth providers\n  cxn auth-broker list",
		"# Local login (run on the broker host)\n  cxn auth-broker login anthropic",
		"# Interactive provider selection\n  cxn auth-broker login",
		"# Remote login over SSH tunnel\n  cxn auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  cxn auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  cxn auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  cxn auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  cxn auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  cxn auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  cxn auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("cxn", "auth-broker", AuthBroker);
			return;
		}
