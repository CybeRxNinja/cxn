/**
 * `cxn browser-relay` — drive the user's own Chrome tabs.
 */
...
export default class BrowserRelay extends Command {
	...
	static flags = {
		...
		dir: Flags.string({
			description: "Extension install directory (install; default ~/.cxn/browser-relay/extension)",
		}),
		"no-group": Flags.boolean({
			description: "Don't gather controllable tabs into an 'cxn' tab group",
			default: false,
		}),
	...
	static examples = [
		"cxn browser-relay install    # write the Chrome extension to disk + setup steps",
		"cxn browser-relay            # serve the relay on the default port",
		"cxn browser-relay -p 9333 --token s3cret",
	];
