...
export default class DryBalance extends Command {
	...
	static flags = {
		model: Flags.string({ description: "Model selector (same syntax as --model on cxn)" }),
	...
	static examples = [
		...
		"# Dry-run a specific model\n  cxn dry-balance anthropic/claude-sonnet-4-5",
		"# Larger run with bounded concurrency\n  cxn dry-balance --model openai-codex/gpt-5-codex --count 1000 --concurrency 64",
		"# Benchmark every OAuth account in parallel\n  cxn dry-balance --bench",
		"# Machine-readable output\n  cxn dry-balance --json",
	];
