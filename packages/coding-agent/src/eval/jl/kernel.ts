...
export class JuliaKernel extends BaseKernel<KernelExecuteOptions> {
	...
	static async start(options: KernelStartOptions): Promise<JuliaKernel> {
		...
		const scriptPath = await stageRunnerScript("cxn-julia-runner", "jl", RUNNER_SCRIPT);
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [
		`__cxn_init_cwd = String(Base64.base64decode("${Buffer.from(cwd).toString("base64")}"))`,
		"try cd(__cxn_init_cwd) catch; end",
	];
	...
	lines.push("if !(__cxn_init_cwd in LOAD_PATH); pushfirst!(LOAD_PATH, __cxn_init_cwd); end");
	return lines.join("\n");
}
