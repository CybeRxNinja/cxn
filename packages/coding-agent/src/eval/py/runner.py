"""CXN Python runner — subprocess wrapper used by the coding-agent host.
...
def _start_capture_drain() -> None:
    ...
    thread = threading.Thread(
        target=_drain_captured_stdout, name="cxn-fd1-capture", daemon=True
    )
    thread.start()
...
def transform_cell(source: str) -> str:
    ...
    Rules
    -----
    * ``%name args``              -> ``__cxn_magic("name", "args")``
    * ``var = %name args``        -> ``var = __cxn_magic("name", "args")``
    * ``!cmd``                    -> ``__cxn_shell("cmd")``
    * ``var = !cmd``              -> ``var = __cxn_shell("cmd")``
    * ``%%name args\\n<body>``    -> ``__cxn_magic_cell("name", "args", "<body>")``
    ...
    while i < len(lines):
        ...
        if stripped.startswith("%%"):
            ...
            body = "\n".join(body_lines)
            out.append(
                f"{indent}__cxn_magic_cell({_quote_arg(name)}, {_quote_arg(args)}, {_quote_arg(body)})"
            )
            return "\n".join(out)
        ...
        if stripped.startswith("%") and not stripped.startswith("%%"):
            ...
            name, args = head
            out.append(f"{indent}__cxn_magic({_quote_arg(name)}, {_quote_arg(args)})")
            i += consumed
            continue
        ...
        if stripped.startswith("!"):
            ...
            out.append(f"{indent}__cxn_shell({_quote_arg(cmd)})")
            i += consumed
            continue
        ...
        if m:
            ...
            if rhs.startswith("!"):
                cmd = rhs[1:].strip()
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __cxn_shell({_quote_arg(cmd)})"
                )
                i += 1
                continue
            if rhs.startswith("%") and not rhs.startswith("%%"):
                ...
                name, args = head
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __cxn_magic({_quote_arg(name)}, {_quote_arg(args)})"
                )
                i += 1
                continue
...
def _emit_status(op: str, **data: Any) -> None:
    bundle = {"application/x-cxn-status": {"op": op, **data}}
    rid = _CURRENT_RID.get()
    if rid is None:
        return
...
def __cxn_magic(name: str, args: str) -> Any:
...
def __cxn_magic_cell(name: str, args: str, body: str) -> Any:
...
def __cxn_shell(cmd: str) -> _ShellResult:
    # stdin=DEVNULL: see _run_shell_body.
    proc = subprocess.Popen(
        cmd,
        shell=True,
...
def __cxn_display(value: Any, *, raw: bool = False, kind: str = "display") -> None:
    if raw:
...
def _install_builtins(ns: dict) -> None:
    ns["display"] = __cxn_display
    ns["__cxn_display"] = __cxn_display
    ns["__cxn_magic"] = __cxn_magic
    ns["__cxn_magic_cell"] = __cxn_magic_cell
    ns["__cxn_shell"] = __cxn_shell
    ns["__cxn_current_run_id__"] = lambda: _CURRENT_RID.get()
...
def _start_parent_watchdog() -> None:
    ...
    thread = threading.Thread(target=watch, name="cxn-parent-watchdog", daemon=True)
    thread.start()
...
async def _handle_request_async(req: dict) -> None:
    ...
    _STATE.user_ns["__cxn_run_id__"] = rid
