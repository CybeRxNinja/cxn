...
# CXN prelude helpers (loaded once into the runner namespace)
if "__cxn_prelude_loaded__" not in globals():
    __cxn_prelude_loaded__ = True
    ...
    # __cxn_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _cxn_display = __cxn_display  # type: ignore[name-defined]
    ...
    def _emit_status(op: str, **data):
        ...
        _cxn_display({"application/x-cxn-status": {"op": op, **data}}, raw=True)
    ...
    _CXN_INTERNAL_URL_RE = re.compile(r"^([a-z][a-z0-9+.-]*)://(.*)$", re.IGNORECASE)

    def _should_delegate_read(path: str | Path) -> bool:
        return (
            isinstance(path, str)
            and _CXN_INTERNAL_URL_RE.match(path) is not None
    ...
    def _resolve_cxn_path(path: str | Path) -> Path:
        ...
        if not isinstance(path, str):
            return Path(path)
        match = _CXN_INTERNAL_URL_RE.match(path)
        if not match:
            return Path(path)
    ...
    def _bridge_call(name: str, args: dict):
        ...
        _run_id_getter = globals().get("__cxn_current_run_id__")
        _run_id = (
            _run_id_getter()
            if callable(_run_id_getter)
            else globals().get("__cxn_run_id__")
        )
        payload = json.dumps(
    ...
    def phase(title):
        ...
        globals()["__cxn_current_phase__"] = str(title)
