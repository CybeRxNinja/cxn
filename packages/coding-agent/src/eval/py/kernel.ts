...
import { $flag, isBunTestRuntime, logger, Snowflake } from "@cyberxninja-omp/pi-utils";
import { $ } from "bun";
...
export class PythonKernel extends BaseKernel {
	...
	static async start(options: KernelStartOptions): Promise<PythonKernel> {
		...
		const scriptPath = await stageRunnerScript("cxn-python-runner", "py", RUNNER_SCRIPT);
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	return [
		"import os, sys",
		`__cxn_cwd = ${JSON.stringify(cwd)}`,
		"os.chdir(__cxn_cwd)",
		`__cxn_env = ${JSON.stringify(envPayload)}`,
		"for __cxn_key, __cxn_val in __cxn_env.items():\n    os.environ[__cxn_key] = __cxn_val",
		"if __cxn_cwd not in sys.path:\n    sys.path.insert(0, __cxn_cwd)",
	].join("\n");
}
