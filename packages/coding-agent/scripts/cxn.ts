/**
 * Bun `--preload` shim for the cxn dev launcher (`scripts/cxn`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the cxn CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@cxn/pi-utils/dirs`).
 */
const launchCwd = process.env.CXN_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.CXN_LAUNCH_CWD;
	try {
