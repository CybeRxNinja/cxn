#!/usr/bin/env bun
/**
 * Build the standalone share-viewer page the cxn relay serves at `GET /s/<id>`.
 *
 ...
 * hands the JSON to template.js via `window.__CXN_SESSION_DATA__`.
 *
...
// Public artifacts use the bundled cxn web themes rather than TUI themes.
const themeStyles = await generateThemeStyles("web");

const html = getTemplate()
	.replace("<theme-vars/>", () => `<style>${themeStyles}</style>`)
	.replace("<title>Session Export</title>", () => "<title>cxn session</title>")
...
if (!html.includes("__CXN_SESSION_DATA__")) throw new Error("share loader not injected");
