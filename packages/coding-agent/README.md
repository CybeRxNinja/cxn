...
Core implementation package for the `cxn` coding agent in the `cxn` monorepo.
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.cxn/config.yml`):
