# Developing `@cxn/pi-coding-agent`

This package is the `cxn` CLI. This file is a **developer's map**: where things live
...
| Build the `dist/cxn` binary | `bun run build` |
...
and dispatches the hidden `__cxn_worker_*` argv selectors before loading the command
