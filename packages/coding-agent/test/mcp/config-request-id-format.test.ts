...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-mcp-reqid-home-"));
	tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-mcp-reqid-agent-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-mcp-reqid-cwd-"));
