 ...
 * shows up in `~/.cxn/logs/cxn.log` without regressing idle-worker shutdown.
 */
import { describe, expect, it } from "bun:test";
import * as path from "node:path";
import { createWorkerSubprocess, type SpawnedSubprocess } from "@cxn/pi-coding-agent/subprocess/worker-client";
...
describe("issue #4324 — worker subprocess stderr survives to the exit error", () => {
	...
	it("does not keep the parent alive while an unref'd worker stays idle", async () => {
		...
		const wrapperScript = `
			const { createWorkerSubprocess } = await import("@cxn/pi-coding-agent/subprocess/worker-client");
