...
import { enforcePlanModeWrite, resolvePlanPath } from "@cyberxninja-omp/pi-coding-agent/tools/plan-mode-guard";
...
describe("resolvePlanPath local:// support", () => {
	...
	it("falls back to os tmp root when artifacts dir is unavailable", () => {
		...
		expect(resolvePlanPath(session, "local://memo.txt")).toBe(
			path.join(os.tmpdir(), "cxn-local", "session-42", "memo.txt"),
		);
	});
});
