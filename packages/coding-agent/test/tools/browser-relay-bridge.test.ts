...
import { RelayBridge, type RelaySocket } from "@cxn/pi-coding-agent/tools/browser/relay/bridge";
import type {
	RelayRpcRequest,
	RelayToExtMessage,
	TabSnapshot,
} from "@cxn/pi-coding-agent/tools/browser/relay/protocol";
...
/**
 * Emulate the cxn tab worker adopting a tab: attach to its page target, then
...
async function claimTab(
	...
	bridge.cdpMessage(connId, JSON.stringify({ id: ++msgSeq, sessionId, method: "CXN.claimTarget" }));
	await flush();
}

describe("RelayBridge tab grouping", () => {
	it("groups nothing on hello or tab lifecycle events — only claimed tabs join the cxn group", () => {
		const bridge = new RelayBridge({ group: { title: "cxn", color: "cyan" } });
	...
	it("groups exactly the tab a client claims", async () => {
		...
		expect(groups[0]!.title).toBe("cxn");
	...
	it("never re-groups a tab the user pulled out of the cxn group", async () => {
		const bridge = new RelayBridge({ group: { title: "cxn", color: "cyan" } });
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
	...
	it("never overlaps group RPCs: a tab claimed mid-flight waits for the pending group", async () => {
		...
		// and mint duplicate "cxn" groups; the second request must queue.
	...
	it("regroups claimed tabs after an extension reconnect instead of treating the dissolve as user opt-out", async () => {
		...
		ack(bridge, ext, "group", { grouped: { "1": 42 } });
		await flush();
		// Relay/extension link drops: the extension dissolves the cxn group on
