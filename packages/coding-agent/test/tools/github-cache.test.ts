 ...
 *
 * Each test isolates `CXN_GITHUB_CACHE_DB` to a temp file and clears
...
beforeEach(async () => {
	...
	originalEnv = process.env.CXN_GITHUB_CACHE_DB;
	process.env.CXN_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.CXN_GITHUB_CACHE_DB;
	} else {
		process.env.CXN_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.CXN_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
