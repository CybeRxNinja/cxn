...
import { KeybindingsManager, setKeyHintPlatform } from "@cyberxninja-omp/pi-coding-agent/config/keybindings";
import { getThemeByName, initTheme, type Theme, theme } from "@cyberxninja-omp/pi-coding-agent/modes/theme/theme";
import {
	dedupeParseErrors,
	expandKeyHint,
	formatCodeFrameLine,
	...
	shortenPath,
	truncateDiffByHunk,
} from "@cyberxninja-omp/pi-coding-agent/tools/render-utils";
import {
	getKeybindings,
	setKeybindings,
...
describe("formatScreenshot", () => {
	...
	it("formats non-home path without tilde", () => {
		const filePath = path.join(path.parse(os.homedir()).root, "cxn-render-utils", "capture.png");
		const resized = fakeResized({ mimeType: "image/webp", buffer: new Uint8Array(1024) });
	...
	it("formats temp-only screenshot without save line", () => {
		...
		expect(
			formatScreenshot({
				...
				dest: path.join(os.tmpdir(), "cxn-sshots-123.png"),
				resized,
			}),
