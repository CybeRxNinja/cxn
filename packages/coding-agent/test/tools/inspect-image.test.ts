...
import { AuthStorage, type completeSimple, Effort, type ImageContent, type Model } from "@cyberxninja-omp/pi-ai";
...
import { InspectImageTool } from "@cyberxninja-omp/pi-coding-agent/tools/inspect-image";
import { inspectImageToolRenderer } from "@cyberxninja-omp/pi-coding-agent/tools/inspect-image-renderer";
import { toolRenderers } from "@cyberxninja-omp/pi-coding-agent/tools/renderers";
import { removeSyncWithRetries, sanitizeText } from "@cyberxninja-omp/pi-utils";
...
describe("InspectImageTool", () => {
	...
	beforeAll(() => {
		testDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-inspect-image-"));
