...
import { createTools, HIDDEN_TOOLS, type ToolSession } from "@cxn/pi-coding-agent/tools";
...
describe("createTools", () => {
	...
	it("still exposes eval when python kernel is unavailable (dispatches to js)", async () => {
		const session = createTestSession();
		vi.spyOn(await import("@cxn/pi-coding-agent/eval/py/kernel"), "checkPythonKernelAvailability").mockResolvedValue({
			ok: false,
