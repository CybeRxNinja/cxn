...
import { downloadFile } from "@cyberxninja-omp/pi-coding-agent/utils/tools-manager";
...
describe("tool asset downloads", () => {
	...
	it("writes a completed response body to disk", async () => {
		using tempDir = TempDir.createSync("@cxn-tool-download-");
	...
	it("aborts a stalled response body and removes the partial file", async () => {
		using tempDir = TempDir.createSync("@cxn-tool-download-stall-");
		const dest = tempDir.join("tool.bin");
