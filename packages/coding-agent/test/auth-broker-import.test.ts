...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.CXN_AGENT_DIR;
		savedEnv.CXN_AUTH_BROKER_URL = process.env.CXN_AUTH_BROKER_URL;
		savedEnv.CXN_AUTH_BROKER_TOKEN = process.env.CXN_AUTH_BROKER_TOKEN;
		delete process.env.CXN_AUTH_BROKER_URL;
		delete process.env.CXN_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.CXN_AGENT_DIR;
		else process.env.CXN_AGENT_DIR = originalAgentDir;
		await removeWithRetries(agentDir);
		await removeWithRetries(cliproxyDir);
		for (const key of ["CXN_AUTH_BROKER_URL", "CXN_AUTH_BROKER_TOKEN"] as const) {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.CXN_AUTH_BROKER_URL = process.env.CXN_AUTH_BROKER_URL;
		savedEnv.CXN_AUTH_BROKER_TOKEN = process.env.CXN_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.CXN_AUTH_BROKER_URL = handle.url;
		process.env.CXN_AUTH_BROKER_TOKEN = token;
	});
