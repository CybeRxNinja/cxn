...
import { BUILTIN_SLASH_COMMANDS } from "@cyberxninja-omp/pi-coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `cxn-move-sibling-${path.basename(tempDir)}`;
