...
import { generateRoomKey, generateWriteToken, importRoomKey, open, seal } from "@cxn/pi-coding-agent/collab/crypto";
import {
	type CollabFrame,
	DEFAULT_RELAY_URL,
	formatCollabLink,
...
describe("collab link format", () => {
	...
	it("accepts legacy #-separated links", () => {
		...
		for (const legacy of [
			...
			`https://my.cxn.sh/#${roomId}#${keyText}`,
		]) {
	...
	it("accepts %23-mangled legacy deep links (macOS Foundation re-encoding)", () => {
		const keyText = Buffer.from(key).toString("base64url");
		const parsed = parseCollabLink(`https://my.cxn.sh/#${roomId}%23${keyText}`);
		if ("error" in parsed) throw new Error(parsed.error);
		expect(parsed.wsUrl).toBe(`${DEFAULT_RELAY_URL}/r/${roomId}`);
		expect(Buffer.from(parsed.key)).toEqual(Buffer.from(key));
	});
	...
	it("parses the scheme-less display form of web deep links", () => {
		const parsed = parseCollabLink(`my.cxn.sh/#${formatCollabLink(DEFAULT_RELAY_URL, roomId, key)}`);
		if ("error" in parsed) throw new Error(parsed.error);
		expect(parsed.wsUrl).toBe(`${DEFAULT_RELAY_URL}/r/${roomId}`);
		expect(Buffer.from(parsed.key)).toEqual(Buffer.from(key));
	});
	...
	it("keeps the key out of web-link path and query", () => {
		...
		expect(url.origin).toBe("https://my.cxn.sh");
