...
import * as SessionSelector from "@cyberxninja-omp/pi-coding-agent/modes/components/session-selector";
...
describe("SelectorController.handleResumeSession preflight flush", () => {
	...
	it("proceeds and returns true when flush succeeds", async () => {
		const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-resume-preflight-"));
		try {
			const { ctx, switchSession, applyCwdChange, state } = createResumeContext({ sourceCwd: tmpDir });
			const targetCwd = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-resume-target-"));
	...
	it("skips flush when settingsFlushed option is true", async () => {
		const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-resume-preflight-skip-"));
		try {
			const { ctx, switchSession, state } = createResumeContext({ sourceCwd: tmpDir });
			const targetCwd = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-resume-target-skip-"));
			switchSession.mockImplementation(async () => {
				state.cwd = targetCwd;
				return true;
			});
