...
import { LoginDialogComponent } from "@cyberxninja-omp/pi-coding-agent/modes/components/login-dialog";
...
describe("SelectorController login", () => {
	...
	it("routes enhanced paste into a direct API-key prompt", async () => {
		...
		dialog.pasteText("CXN_PASTE_TEST_123");
		dialog.handleInput("\n");

		await expect(prompt).resolves.toBe("CXN_PASTE_TEST_123");
	});
});
