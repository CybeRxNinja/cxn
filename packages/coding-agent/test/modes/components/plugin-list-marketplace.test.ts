...
import { PluginManager } from "@cxn/pi-coding-agent/extensibility/plugins";
import {
...
} from "@cxn/pi-coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("cxn plugin install <package>");
		expect(text).toContain("cxn plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.cxn/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.cxn/cache/plugins/sample@mkt");
