...
import { parseArgs, reportUnrecognizedFlags } from "@cxn/pi-coding-agent/cli/args";
import { applyExtensionFlags } from "@cxn/pi-coding-agent/cli/extension-flags";

// Regression coverage for issue #2459: `cxn --list-models` (a stale flag) was
...
describe("parseArgs — unrecognized flag tracking (#2459)", () => {
	...
	it("treats every token after `--` as a positional, even flag-shaped ones (#2461 review)", () => {
		// `cxn -p -- --explain-this` must forward `--explain-this` as the
