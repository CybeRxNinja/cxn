...
const CXN_AGENT_MD = [
	"---",
	"name: cxn-test-agent",
	"description: CXN-native test agent.",
	"---",
	"You are an CXN task agent.",
].join("\n");

const CXN_PLUGIN_AGENT_MD = [
	"---",
...
async function writeOmpPluginAgent(home: string): Promise<void> {
	...
	await fs.writeFile(
		...
		JSON.stringify({
			name: "cxn-plugins-root",
			version: "0.0.0",
			dependencies: { loom: "1.0.0" },
		}),
	);
	await fs.writeFile(path.join(pluginRoot, "agents", "loom-verify-spec.md"), CXN_PLUGIN_AGENT_MD);
}
...
describe("discoverAgents", () => {
	...
	beforeEach(async () => {
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-task-agent-discovery-"));
		projectDir = path.join(tempHome, "project");
		await fs.mkdir(projectDir, { recursive: true });
	});

	afterEach(async () => {
		enableProvider("cxn-plugins");
		clearOmpExtensionCliRoots();
		clearFsCache();
	...
	test("excludes CXN npm plugin agents when cxn-plugins is disabled", async () => {
		await writeOmpPluginAgent(tempHome);
		disableProvider("cxn-plugins");
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		// the skills/hooks/tools surface in discovery/cxn-plugins.ts.
