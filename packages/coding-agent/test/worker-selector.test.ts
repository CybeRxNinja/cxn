...
// The worker-host re-entry seam dispatches any `__cxn_worker_*` selector to
...
describe("worker selector dispatch", () => {
	...
	it("fails with a nonzero exit and stderr error on an unknown selector", async () => {
		...
		await runCli(["__cxn_worker_does_not_exist"]);

		expect(process.exitCode).toBe(1);
		expect(stderr).toHaveBeenCalledWith("Error: unknown worker selector: __cxn_worker_does_not_exist\n");
	});
