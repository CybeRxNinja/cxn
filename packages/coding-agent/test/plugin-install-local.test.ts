...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-plugin-install-local-"));
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "cxn-plugins.lock.json"));
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "cxn-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
