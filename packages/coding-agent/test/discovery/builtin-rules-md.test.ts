 ...
 * from both `~/.cxn/agent/RULES.md` (user) and the nearest `.cxn/RULES.md`
...
import { type Rule, ruleCapability } from "@cxn/pi-coding-agent/capability/rule";
...
test("user ~/.cxn/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".cxn", "agent", "RULES.md"),
...
test("project .cxn/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".cxn", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".cxn", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".cxn", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".cxn", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".cxn", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".cxn", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .cxn/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".cxn", "agent", "rules", "other.md"), "# Unrelated rule\n");
