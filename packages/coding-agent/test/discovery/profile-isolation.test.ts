/**
 * Regression: CXN-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.cxn/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.cxn/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".cxn/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@cxn/pi-coding-agent/capability/skill";
import { type SlashCommand, slashCommandCapability } from "@cxn/pi-coding-agent/capability/slash-command";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-profile-iso-agent-"));
