...
import { expandAtImports, MAX_AT_IMPORT_DEPTH } from "@cyberxninja-omp/pi-coding-agent/discovery/at-imports";
...
describe("expandAtImports", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-at-import-"));
	});
	...
	test("resolves ~/path against the home override", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-at-home-"));
		try {
