 ...
 * resolved those relative values against the CXN session cwd, so the bundled Codex
 ...
 * `resolvePluginStdioPaths`, matching the claude-plugins/cxn-plugins fix in #5481.
 */
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-codex-mcp-home-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-codex-mcp-cwd-"));
