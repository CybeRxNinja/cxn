...
import { getConfigDirs } from "@cxn/pi-coding-agent/config";
import { resolveClaudePaths } from "@cxn/pi-coding-agent/config/claude-paths";
import { getUserPath } from "@cxn/pi-coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".cxn/agent" and leaked the default
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		process.env.PI_CONFIG_DIR = ".config/cxn";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/cxn", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".cxn", level: "user" });
	});
});
