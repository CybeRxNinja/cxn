...
import { type Settings, settingsCapability } from "@cyberxninja-omp/pi-coding-agent/capability/settings";
...
describe("OpenCode MCP discovery", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-opencode-mcp-"));
