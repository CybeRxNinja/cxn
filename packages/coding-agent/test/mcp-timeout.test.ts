...
import { isMCPTimeoutEnabled, resolveMCPTimeoutMs } from "@cyberxninja-omp/pi-coding-agent/mcp/timeout";
import { logger } from "@cyberxninja-omp/pi-utils";

const ORIGINAL_TIMEOUT = process.env.CXN_MCP_TIMEOUT_MS;

afterEach(() => {
	if (ORIGINAL_TIMEOUT === undefined) {
		delete process.env.CXN_MCP_TIMEOUT_MS;
	} else {
		process.env.CXN_MCP_TIMEOUT_MS = ORIGINAL_TIMEOUT;
	}
});
...
describe("MCP timeout configuration", () => {
	...
	test("allows the env override to disable MCP client-side timeouts", () => {
		process.env.CXN_MCP_TIMEOUT_MS = "0";
	...
	test("rejects non-numeric env values and falls back to the default", () => {
		process.env.CXN_MCP_TIMEOUT_MS = "not-a-number";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});
