 ...
 * `process.exit()` at the top level must not terminate the host CXN process.
 *
 * The harness intercepts the load via `withHostGuard`; this test pins that the
 * intercepted error surfaces as a per-module load failure (so CXN keeps going)
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@cxn-exit-guard-");
	});
