...
import { __buildLegacyPiPackageRootOverrides } from "@cxn/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
`,
	...
	it("serves @cxn/pi-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cxn/pi-ai/oauth"]).toBe("cxn-legacy-pi-bundled:@cxn/pi-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cxn/pi-ai/oauth/anthropic"]).toBe("cxn-legacy-pi-bundled:@cxn/pi-ai/oauth/anthropic");
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@cxn/pi-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@cxn/pi-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@cxn/pi-ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@cxn/pi-ai/providers/cursor-pi-args";
...
`,
	...
	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@cxn/pi-coding-agent/web/search/providers/xai",
			"@cxn/pi-coding-agent/web/search/providers/tinyfish",
			"@cxn/pi-coding-agent/web/search/providers/firecrawl",
			"@cxn/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@cxn/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@cxn/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@cxn/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@cxn/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			...
			// TYPEBOX_SHIM_PATH.
			if (key === "@cxn/pi-ai" || key === "@cxn/pi-coding-agent" || key === "@cxn/pi-tui" || key === "typebox")
				continue;
	...
	it("keeps pi-ai/pi-coding-agent/pi-tui roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cxn/pi-ai"]).toBeDefined();
		expect(overrides["@cxn/pi-ai"]).not.toBe("cxn-legacy-pi-bundled:@cxn/pi-ai/oauth");
		expect(overrides["@cxn/pi-coding-agent"]).toBeDefined();
		expect(overrides["@cxn/pi-tui"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@cxn/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@cxn/pi-coding-agent/tools");
	...
	it("bundles nested wildcard subpaths so a compiled extension can import them", () => {
		...
		expect(bundledModuleKeys.has("@cxn/pi-coding-agent/slash-commands/helpers/active-oauth-account")).toBe(true);
		// Directory index modules stay excluded: `./x/*` must not serve `x/y`
		// from `y/index.ts`, which Node would not resolve either.
		expect(bundledModuleKeys.has("@cxn/pi-coding-agent/modes/theme/defaults/index")).toBe(false);
	});
});
