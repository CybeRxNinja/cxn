...
import { __buildLegacyPiPackageRootOverrides } from "@cyberxninja-omp/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
// the same `cxn-legacy-pi-bundled:` virtual namespace as package roots without
...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("does not evaluate unrelated host modules while loading the registry", async () => {
		using tempDir = TempDir.createSync("@cxn-legacy-pi-loaders-");
...
`,
	...
	it("serves @cyberxninja-omp/pi-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cyberxninja-omp/pi-ai/oauth"]).toBe("cxn-legacy-pi-bundled:@cyberxninja-omp/pi-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cyberxninja-omp/pi-ai/oauth/anthropic"]).toBe(
			"cxn-legacy-pi-bundled:@cyberxninja-omp/pi-ai/oauth/anthropic",
		);
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@cyberxninja-omp/pi-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@cyberxninja-omp/pi-ai/providers/cursor-pi-args";
...
`,
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides[key]).toBe(`cxn-legacy-pi-bundled:${key}`);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@cyberxninja-omp/pi-coding-agent/web/search/providers/xai",
			"@cyberxninja-omp/pi-coding-agent/web/search/providers/tinyfish",
			"@cyberxninja-omp/pi-coding-agent/web/search/providers/firecrawl",
			"@cyberxninja-omp/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@cyberxninja-omp/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			...
			// TYPEBOX_SHIM_PATH.
			if (
				key === "@cyberxninja-omp/pi-ai" ||
				key === "@cyberxninja-omp/pi-coding-agent" ||
				key === "@cyberxninja-omp/pi-tui" ||
				key === "typebox"
			)
				continue;
			if (overrides[key] !== `cxn-legacy-pi-bundled:${key}`) {
				missing.push(key);
			}
		}
	...
	it("keeps pi-ai/pi-coding-agent/pi-tui roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@cyberxninja-omp/pi-ai"]).toBeDefined();
		expect(overrides["@cyberxninja-omp/pi-ai"]).not.toBe("cxn-legacy-pi-bundled:@cyberxninja-omp/pi-ai/oauth");
		expect(overrides["@cyberxninja-omp/pi-coding-agent"]).toBeDefined();
		expect(overrides["@cyberxninja-omp/pi-tui"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@cyberxninja-omp/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@cyberxninja-omp/pi-coding-agent/tools");
	...
	it("bundles nested wildcard subpaths so a compiled extension can import them", () => {
		...
		expect(
			bundledModuleKeys.has("@cyberxninja-omp/pi-coding-agent/slash-commands/helpers/active-oauth-account"),
		).toBe(true);
		// Directory index modules stay excluded: `./x/*` must not serve `x/y`
		// from `y/index.ts`, which Node would not resolve either.
		expect(bundledModuleKeys.has("@cyberxninja-omp/pi-coding-agent/modes/theme/defaults/index")).toBe(false);
	});
});
