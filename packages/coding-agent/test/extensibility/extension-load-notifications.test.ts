...
import { formatExtensionLoadNotifications } from "@cyberxninja-omp/pi-coding-agent/extensibility/extensions/load-errors";
...
describe("extension load startup notifications", () => {
	it("formats load failures as sanitized single-line warnings for TUI and print startup paths", () => {
		...
		const extensionPath = path.join(homeDir, "cxn-notification-fixture", "plugin\tname", "extension.ts");
		...
		expect(message?.startsWith("Failed to load extension ~/cxn-notification-fixture/plugin")).toBe(true);
