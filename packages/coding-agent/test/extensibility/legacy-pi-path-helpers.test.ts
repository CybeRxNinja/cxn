...
import * as configModule from "@cxn/pi-coding-agent/config";
import * as shim from "@cxn/pi-coding-agent/extensibility/legacy-pi-coding-agent-shim";
import * as utils from "@cxn/pi-utils";
...
describe("legacy shim path helpers", () => {
	...
	it("getPackageDir resolves the coding-agent package root in source mode", () => {
		// cxn's canonical helper returns the package root containing package.json
	...
	// `path.join(getPackageDir(), ...)`. cxn's canonical helper returns
