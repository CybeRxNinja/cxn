...
// `./core/compaction/index.ts`); in cxn it lives in
// `@cxn/pi-agent-core/compaction` and the coding-agent barrel does not
...
// during plugin validation (e.g. `cxn plugin install pi-blackhole`). This pins
...
describe("legacy shim compaction helpers", () => {
	...
	// Issue #7174: `compact` (same `@cxn/pi-agent-core/compaction` module as
	// `estimateTokens`) was likewise absent from the shim surface, so
	// `cxn plugin install npm:pi-claude-bridge` failed with "Export named
