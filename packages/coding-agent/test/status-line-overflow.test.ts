...
describe("path segment truncation at varying maxLength", () => {
	...
	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-overflow-very-long-directory-name-for-testing-"));
...
describe("overflow: path shrinks before git is dropped", () => {
	...
	beforeAll(() => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-overflow-a-very-long-worktree-directory-name-here-"));
		setProjectDir(tmpDir);
	});
	...
	it("shrinks a short path when maxLength exceeds actual path length", () => {
		...
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-short-"));
	...
	it("preserves git when overflow is only 1-2 columns", () => {
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-narrow-ovf-"));
		setProjectDir(shortDir);
		try {
...
describe("overflow: path survives before model", () => {
	it("drops the model segment before the cwd path when both cannot fit", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-statusline-overflow-"));
