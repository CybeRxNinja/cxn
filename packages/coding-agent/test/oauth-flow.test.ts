...
import { MCPOAuthFlow, refreshMCPOAuthToken } from "@cyberxninja-omp/pi-coding-agent/mcp/oauth-flow";
...
describe("mcp oauth flow", () => {
	it("uses omp client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("omp");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'omp' is not on the approved list.",
					}),
