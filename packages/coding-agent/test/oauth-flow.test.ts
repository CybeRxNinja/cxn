...
import { MCPOAuthFlow, refreshMCPOAuthToken } from "@cxn/pi-coding-agent/mcp/oauth-flow";
...
describe("mcp oauth flow", () => {
	it("uses cxn client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("cxn");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'cxn' is not on the approved list.",
					}),
