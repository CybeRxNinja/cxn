...
test("getEnabledPlugins caches repeated discovery for the same cwd and home until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-plugin-cache-"));
	tempRoots.push(root);
	...
	await writeJson(path.join(pluginsDir, "package.json"), { dependencies: { "cxn-cache-repro": "1.0.0" } });
	await writeJson(path.join(pluginsDir, "cxn-plugins.lock.json"), {
		plugins: { "cxn-cache-repro": { version: "1.0.0", enabled: true, enabledFeatures: null } },
		settings: {},
	});
...
test("legacy bare dependency rewrites cache fallback package resolution until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-legacy-cache-"));
	tempRoots.push(root);
