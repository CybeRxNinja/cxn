...
import {
	...
	type SessionEvent,
} from "@cyberxninja-omp/pi-coding-agent/extensibility/hooks";
import { theme } from "@cyberxninja-omp/pi-coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `cxn-compaction-hooks-test-${Snowflake.next()}`);
