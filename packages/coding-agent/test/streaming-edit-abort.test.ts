...
import type { AssistantMessage, StopReason, ToolCall } from "@cyberxninja-omp/pi-ai";
...
import * as autoGeneratedGuard from "@cyberxninja-omp/pi-coding-agent/tools/auto-generated-guard";
...
it("keeps the session alive when replace mode streams an ssh:// path", async () => {
	const checkSpy = vi.spyOn(autoGeneratedGuard, "assertEditableFile");
	const abortSignalRef: { current?: AbortSignal } = {};
	const remotePath = "ssh://test-host/tmp/cxn-repro.txt";
