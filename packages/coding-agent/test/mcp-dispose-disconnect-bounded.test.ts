 ...
 * default, unbounded when `CXN_MCP_TIMEOUT_MS=0`). `dispose()` wraps the
...
import { removeSyncWithRetries, withTimeout } from "@cyberxninja-omp/pi-utils";
...
describe("owned-manager dispose disconnect is bounded (PR #2839)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-mcp-dispose-"));
	});
