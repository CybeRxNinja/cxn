...
describe("profile alias installer", () => {
	...
	it("can target the current source invocation instead of the installed cxn binary", async () => {
		...
		expect(files.get("/Users/me/.zshrc")).toContain("cxn-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("cxn-work() {");
	});
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid CXN profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/cxn-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/cxn-profiles.fish'");
	});
