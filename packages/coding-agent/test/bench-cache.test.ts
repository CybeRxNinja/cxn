...
import { type BenchModelRegistry, runBenchCommand } from "@cyberxninja-omp/pi-coding-agent/cli/bench-cli";
...
describe("bench cache mode", () => {
	it("splits the cache breakpoint prefix from each variable suffix with native CXN messages", async () => {
	...
	it("truncates the default prefix-file reader at a UTF-8 boundary before decoding", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-bench-cache-prefix-"));
	...
	it("preserves significant whitespace and replacement patterns from the default prefix-file reader", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-bench-cache-prefix-whitespace-"));
		const prefixPath = path.join(tempDir, "prefix.txt");
