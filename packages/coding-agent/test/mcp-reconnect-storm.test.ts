...
import type { MCPStdioServerConfig } from "@cyberxninja-omp/pi-coding-agent/mcp/types";
...
describe("MCP reconnect storm (issue #1592)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-mcp-storm-"));
	...
	it("stops respawning after a burst of immediate exits", async () => {
		...
		const config: MCPStdioServerConfig = {
			...
			command: BUN_EXEC,
			args: [FIXTURE_PATH],
			env: { CXN_TEST_SPAWN_LOG: spawnLog },
		};
