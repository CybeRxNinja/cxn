...
describe("global --profile flag", () => {
	...
	beforeEach(() => {
		...
		configDir = `.cxn-profile-cli-test-${Snowflake.next()}`;
	...
	it("activates a profile inherited from CXN_PROFILE at run time", async () => {
		const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		setProfile(undefined);
		process.env.CXN_PROFILE = "work";
		delete process.env.PI_PROFILE;
	...
	it("installs a shell alias and exits before command dispatch", async () => {
		...
		await runCli(["--profile", "work", "--alias", "cxn-work", "--version"]);
		...
		expect(output).toContain("Created cxn-work");
	...
	it("installs a shell alias when launch is explicit", async () => {
		...
		await runCli(["launch", "--profile", "work", "--alias", "cxn-work", "--version"]);
	...
	it("installs a shell alias when acp is explicit", async () => {
		...
		await runCli(["acp", "--profile", "work", "--alias", "cxn-work", "--version"]);
	...
	it("loads profile agent .env before command modules import pi-utils env", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-profile-cli-env-"));
		try {
			const home = path.join(root, "home");
			const configDir = ".cxn-profile-cli-env";
			...
			await Bun.write(path.join(defaultAgentDir, ".env"), "CXN_PROFILE_BOOTSTRAP_SENTINEL=default\n");
			await Bun.write(path.join(profileAgentDir, ".env"), "CXN_PROFILE_BOOTSTRAP_SENTINEL=work\n");
			...
			await Bun.write(
				...
				[
					...
					'process.stdout.write("\\nSENTINEL=" + (Bun.env.CXN_PROFILE_BOOTSTRAP_SENTINEL ?? ""));',
				].join("\n"),
			);
			...
			delete childEnv.CXN_PROFILE;
			delete childEnv.PI_PROFILE;
			delete childEnv.PI_CODING_AGENT_DIR;
			delete childEnv.CXN_PROFILE_BOOTSTRAP_SENTINEL;
	...
	it("surfaces an invalid CXN_PROFILE env as a clean error, not an import crash", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-profile-cli-env-bad-"));
		try {
			const home = path.join(root, "home");
			...
			const childEnv: Record<string, string | undefined> = {
				...process.env,
				HOME: home,
				PI_CONFIG_DIR: ".cxn-profile-cli-env-bad",
				CXN_PROFILE: "..",
				NO_COLOR: "1",
			};
			delete childEnv.PI_PROFILE;
			...
			expect(stderr).toContain("Invalid CXN profile");
