...
describe("PluginManager.install load validation", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-plugin-validation-"));
		...
		vi.spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "cxn-plugins.lock.json"));
	...
	test("rejects an install whose extension entry cannot resolve its dependencies", async () => {
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "cxn-plugins", private: true, dependencies: { "broken-plugin": "1.0.0" } },
						null,
						2,
					),
				);
		...
		expect(await Bun.file(path.join(tmpRoot, "cxn-plugins.lock.json")).exists()).toBe(false);
	});

	test("restores the previous package tree when reinstall validation fails", async () => {
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "cxn-plugins", private: true, dependencies: { "broken-plugin": "1.0.0" } }, null, 2),
		);
		await Bun.write(
			path.join(tmpRoot, "cxn-plugins.lock.json"),
			JSON.stringify(
		...
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "cxn-plugins", private: true, dependencies: { "broken-plugin": "2.0.0" } },
						null,
						2,
					),
				);
		...
		const lock = await Bun.file(path.join(tmpRoot, "cxn-plugins.lock.json")).json();
	...
	test("restores the previous git plugin tree when reinstalling a different ref fails validation", async () => {
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify(
				{ name: "cxn-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin#v1" } },
				null,
				2,
			),
		);
			...
			if (cmd[1] === "install") {
				...
				const prepare = (async () => {
					await Bun.write(
						pluginsPkgJson,
						JSON.stringify(
							{ name: "cxn-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin#v2" } },
							null,
							2,
						),
					);
		...
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "cxn-plugins", private: true, dependencies: { "partial-plugin": "1.0.0" } },
						null,
						2,
					),
				);
			...
			if (cmd[1] === "install") {
				const prepare = (async () => {
					...
					await Bun.write(
						pluginsPkgJson,
						JSON.stringify(
							{ name: "cxn-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin" } },
							null,
							2,
						),
					);
