...
beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-discovery-test-"));
	ompRegistryPath = path.join(tmpHome, CXN_CONFIG_DIR, "plugins", "installed_plugins.json");
...
describe("CXN registry format compatibility with Claude parser", () => {
...
describe("CXN registry round-trip", () => {
...
//
// listClaudePluginRoots must replace Claude entries with CXN entries when the same
...
describe("CXN precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — CXN entry has required fields for deduplication", () => {
