...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-fetcher-test-"));
	});
	...
	it("loads catalog from .cxn-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "cxn-only");
		fs.mkdirSync(path.join(root, ".cxn-plugin"), { recursive: true });
		const catalog = {
			name: "cxn-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "cxn-plugin", source: "./plugins/cxn-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".cxn-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("cxn-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("cxn-plugin");
	});

	it("prefers .cxn-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".cxn-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const ompCatalog = {
			name: "from-cxn-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".cxn-plugin", "marketplace.json"), JSON.stringify(ompCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-cxn-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .cxn-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.cxn-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
