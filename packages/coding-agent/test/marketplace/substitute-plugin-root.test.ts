...
import { substitutePluginRoot } from "@cyberxninja-omp/pi-coding-agent/discovery/substitute-plugin-root";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces CXN_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${CXN_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${CXN_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
