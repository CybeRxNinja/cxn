...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "cxn-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "cxn-plugins.lock.json")).json();
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		...
		try {
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "cxn-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "cxn-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "cxn-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "cxn-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-mgr-noproj-"));
		try {
