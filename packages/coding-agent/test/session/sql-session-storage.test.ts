...
import { SqlSessionStorage, type SqlSessionStorageClient } from "@cxn/pi-coding-agent/session/sql-session-storage";
import { SQL } from "bun";
...
describe("SqlSessionStorage (SQLite backend)", () => {
	it("indexes writeText metadata and reads content asynchronously", async () => {
		...
		const rows = (await client.unsafe(`SELECT path, content FROM cxn_session_files WHERE path = ?`, [
			"/sessions/p/a.jsonl",
	...
	it("create() warms the metadata index without selecting full content", async () => {
		const client = new SQL("sqlite::memory:");
		await client.unsafe(
			`CREATE TABLE cxn_session_files (path TEXT PRIMARY KEY, content TEXT NOT NULL, mtime_ms INTEGER NOT NULL, title TEXT, title_source TEXT, title_updated_at TEXT)`,
		);
		await client.unsafe(`INSERT INTO cxn_session_files (path, content, mtime_ms) VALUES (?, ?, ?)`, [
	...
	it("writer.append appends to SQL after drain", async () => {
		...
		await storage.drain();
		const rows = (await client.unsafe(`SELECT content FROM cxn_session_files WHERE path = ?`, [
	...
	it("drain() surfaces writer errors so background failures are observable", async () => {
		...
		await client.unsafe("DROP TABLE cxn_session_files");
	...
	it("deleteSessionWithArtifacts removes JSONL plus any sidecar keys", async () => {
		...
		const remaining = (await client.unsafe(`SELECT path FROM cxn_session_files ORDER BY path`)) as Array<{
			path: string;
		}>;
	...
	it("rename moves content and mtime atomically inside the mirror and the DB", async () => {
		...
		const rows = (await client.unsafe(`SELECT path, content FROM cxn_session_files`)) as Array<{
			path: string;
			content: string;
		}>;
	...
	it("LIKE special chars in artifact paths don't blow up the prefix sweep", async () => {
		...
		const remaining = (await client.unsafe(`SELECT path FROM cxn_session_files`)) as Array<{ path: string }>;
