...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-cwd-rel-"));
