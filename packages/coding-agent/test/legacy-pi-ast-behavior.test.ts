...
beforeAll(async () => {
	...
	rewriteRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "cxn-legacy-ast-rewrite-")));
...
async function loadCommonJsCase(testCase: CommonJsCase): Promise<{ keys: string[]; named: Record<string, unknown> }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-legacy-ast-exports-"));
	tempRoots.push(dir);
