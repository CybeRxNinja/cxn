...
import { resetActiveSkillsForTests, type Skill, setActiveSkills } from "@cxn/pi-coding-agent/extensibility/skills";
import type { HindsightSessionState } from "@cxn/pi-coding-agent/hindsight/state";
import type { MnemopiSessionState } from "@cxn/pi-coding-agent/mnemopi/state";
import { createTools, type ToolSession } from "@cxn/pi-coding-agent/tools";
import { LearnTool } from "@cxn/pi-coding-agent/tools/learn";
import { ManageSkillTool } from "@cxn/pi-coding-agent/tools/manage-skill";
...
describe("manage_skill execute", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-manage-skill-"));
...
describe("learn execute", () => {
	...
	beforeEach(async () => {
		originalAgentDir = getAgentDir();
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-learn-"));
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".cxn", "agent"));
		remembered = [];
	});
