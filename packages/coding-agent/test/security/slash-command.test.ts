...
import { refreshDirsFromEnv } from "@cyberxninja-omp/pi-utils";
...
beforeEach(async () => {
	temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-security-slash-"));
...
describe("/security", () => {
	...
	test("validate returns a static CXN-native residual prompt", async () => {
