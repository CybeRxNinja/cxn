...
import type { AgentSideConnection, SessionNotification } from "@cyberxninja-omp/pi-utils/acp";
...
} from "@cyberxninja-omp/pi-coding-agent/modes/acp/acp-event-mapper";
...
describe("ACP event mapper", () => {
	...
	it("replays assistant tool_use input through the ACP dispatcher without wrapping", async () => {
		const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "cxn-acp-replay-contract-"));
