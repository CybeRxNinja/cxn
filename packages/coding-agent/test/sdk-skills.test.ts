...
import { getActiveSkills } from "@cxn/pi-coding-agent/extensibility/skills";
import type { Skill } from "@cxn/pi-coding-agent/sdk";
...
function createExtensionSkill(packageDir: string, skillName: string): void {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ name: path.basename(packageDir), cxn: { extensions: ["./extension.ts"] } }),
	);
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .cxn/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".cxn", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".cxn", "agent", "skills");
...
`,
	...
	it("SDK invocation root scope isolates disabled discovery and merges normal discovery", async () => {
		...
		const installedPackage = path.join(tempHomeDir, ".cxn", "plugins", "node_modules", "sdk-installed-extension");
		...
		fs.writeFileSync(path.join(tempDir, ".cxn", "settings.json"), JSON.stringify({ extensions: [settingsPackage] }));
		fs.mkdirSync(path.join(tempHomeDir, ".cxn", "plugins"), { recursive: true });
		fs.writeFileSync(
			path.join(tempHomeDir, ".cxn", "plugins", "package.json"),
			JSON.stringify({ name: "cxn-plugins", dependencies: { "sdk-installed-extension": "1.0.0" } }),
		);
		...
		const baseSessionOptions = {
			cwd: tempDir,
			agentDir: path.join(tempHomeDir, ".cxn", "agent"),
			modelRegistry: sharedModelRegistry,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".cxn", "agent");
	...
	it("refreshSkills reloads project skills on an existing session", async () => {
		...
		const runtimeSkillDir = path.join(tempDir, ".cxn", "skills", "runtime-added-skill");
...
`,
	...
	it("manage_skill hot-registers managed skills in the active session", async () => {
		...
		const managedAgentDir = path.join(tempHomeDir, ".cxn", "agent");
