 ...
 *
 * Strategy: spy on the six `@cyberxninja-omp/pi-utils` plugin-path getters so the
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-plugin-git-"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "cxn-plugins", private: true, dependencies: {} }, null, 2),
		);
