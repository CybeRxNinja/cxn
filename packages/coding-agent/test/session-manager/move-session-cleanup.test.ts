...
import { cleanupEmptyMoveSession, SessionManager } from "@cyberxninja-omp/pi-coding-agent/session/session-manager";
...
describe("move-session cleanup tracking", () => {
	...
	beforeEach(async () => {
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "cxn-move-cleanup-"));
