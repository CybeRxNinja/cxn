...
import { listSessions } from "@cyberxninja-omp/pi-coding-agent/session/session-listing";
...
describe("large session memory guards", () => {
	...
	it("streams large session files without discarding historical compactions", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "cxn-large-session-"));
	...
	it("preserves loaded compactions on every branch", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "cxn-branch-load-"));
		tempDirs.push(tempDir);
