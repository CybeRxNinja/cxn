...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-gpu-probe-"));
	try {
		const binDir = path.join(tempRoot, "bin");
		const cacheRoot = path.join(tempRoot, "cache");
		const probeCountPath = path.join(tempRoot, "probe-count");
		await fs.mkdir(binDir, { recursive: true });
		await fs.mkdir(path.join(cacheRoot, "cxn"), { recursive: true });
		const lspciPath = path.join(binDir, "lspci");
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$CXN_GPU_PROBE_COUNT"\nif [ -n "$CXN_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$CXN_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$CXN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$CXN_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$CXN_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$CXN_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$CXN_GPU_PROBE_SLEEP" ]; then exec sleep "$CXN_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.CXN_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.CXN_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			CXN_GPU_PROBE_COUNT: probeCountPath,
			CXN_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		for (const key of ["PI_CODING_AGENT_DIR", "CXN_PROFILE", "PI_PROFILE", "PI_CONFIG_DIR"]) {
			delete env[key];
		}
		if (options.sleepSeconds === undefined) {
			delete env.CXN_GPU_PROBE_SLEEP;
		} else {
			env.CXN_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.CXN_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.CXN_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.CXN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.CXN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.CXN_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.CXN_GPU_PROBE_VALID_OUTPUT;
		}
