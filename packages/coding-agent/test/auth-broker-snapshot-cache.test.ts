...
import { discoverAuthStorage } from "@cyberxninja-omp/pi-coding-agent/sdk";
import { removeWithRetries } from "@cyberxninja-omp/pi-utils";

const ENV_KEYS = [
	"CXN_AUTH_BROKER_URL",
	"CXN_AUTH_BROKER_TOKEN",
	"CXN_AUTH_BROKER_SNAPSHOT_CACHE",
	"CXN_AUTH_BROKER_SNAPSHOT_TTL_MS",
] as const;
...
describe("discoverAuthStorage auth-broker snapshot cache", () => {
	...
	test("boots from a fresh encrypted cache when the broker is down", async () => {
		...
		process.env.CXN_AUTH_BROKER_URL = downUrl;
		process.env.CXN_AUTH_BROKER_TOKEN = TOKEN;
		process.env.CXN_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
		process.env.CXN_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
	...
	test("boots from a fresh cache when revalidation returns a server error", async () => {
		...
		let storage: AuthStorage | undefined;
		try {
			process.env.CXN_AUTH_BROKER_URL = url;
			process.env.CXN_AUTH_BROKER_TOKEN = TOKEN;
			process.env.CXN_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
			process.env.CXN_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
			await writeAuthBrokerSnapshotCache({
