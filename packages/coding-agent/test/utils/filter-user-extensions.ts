...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@cxn/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.cxn/`), plus
//    the legacy non-XDG `~/.cxn/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.cxn/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/cxn/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.cxn` when
...
// symlinks `~/.cxn/agent/extensions/foo.ts` to a dotfiles repo would see the
