...
import { type Skill as CapabilitySkill, skillCapability } from "@cyberxninja-omp/pi-coding-agent/capability/skill";
import { getCapability } from "@cyberxninja-omp/pi-coding-agent/discovery";
import { getWslWindowsHomeCandidate, runHostProbe } from "@cyberxninja-omp/pi-coding-agent/discovery/agents";
import {
	...
	type Skill,
} from "@cyberxninja-omp/pi-coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// canonical CXN-native `~/.agent[s]/skills` (the `agents` provider).
		...
		// MUST NOT consider the CXN-native `enableAgentsUser`/`...Project`
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-dmi-skill-"));
