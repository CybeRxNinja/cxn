...
import { getOrCreateSnapshot, sanitizeSnapshotForBrush } from "@cxn/pi-coding-agent/utils/shell-snapshot";
...
function snapshotDirIn(tmpRoot: string): string {
	...
	return path.join(tmpRoot, uid === undefined ? "cxn-shell-snapshots" : `cxn-shell-snapshots-${uid}`);
}
...
describe("sanitizeSnapshotForBrush", () => {
	...
	it("leaves non-alias lines (functions, exports, unalias) untouched", () => {
		const snapshot = [
			"# Shell snapshot - generated by cxn agent",
			"unalias -a 2>/dev/null || true",
			"my_fn () { echo hi; }",
...
// `__cxn_emit_referenced_exports` (shell-snapshot-fn-env.sh) re-exports env
...
describe("getOrCreateSnapshot", () => {
	it("re-exports env vars referenced by snapshotted functions (issue #3470)", async () => {
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-3470-"));
		await fs.writeFile(
		...
		const shellLink = path.join(home, "bash-cxn-3470");
		...
		// reason — UUID filenames shouldn't leak via `ls /tmp/cxn-shell-snapshots-$(id -u)`.
	...
	it("keeps the snapshot file at 0600 even when the rc file resets umask to 022", async () => {
		...
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-umask-"));
		await fs.writeFile(
			path.join(home, ".bashrc"),
		...
		const realBash = REAL_BASH;
		if (!existsSync(realBash)) return;
		const shellLink = path.join(home, "bash-cxn-umask");
		await fs.symlink(realBash, shellLink);
	...
	it("cleans up the empty snapshot file when the shell exits with a non-zero code", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-fail-"));
	...
	it("cleans up the empty snapshot file when the shell fails to spawn", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-spawn-fail-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
	...
	it("cleans up the empty snapshot file when the shell execution times out", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-timeout-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
	...
	it("keeps snapshots in a uid-scoped dir so accounts sharing /tmp cannot collide", async () => {
		// Regression: the dir used to be a single fixed `cxn-shell-snapshots` name
		// under the shared `os.tmpdir()`, created 0700. The first account to run cxn
		// owned it and every other account's pre-create write died with EACCES.
		const realBash = REAL_BASH;
		if (!existsSync(realBash)) return;
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-uid-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
			const shellLink = path.join(testRoot, "bash-cxn-uid");
			await fs.symlink(realBash, shellLink);
	...
	it("returns null instead of throwing when the snapshot dir is unusable", async () => {
		...
		const realBash = REAL_BASH;
		if (!existsSync(realBash)) return;
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cxn-snap-eacces-"));
		const originalTmpDir = process.env.TMPDIR;
		const shellLink = path.join(testRoot, "bash-cxn-eacces");
		await fs.symlink(realBash, shellLink);
		process.env.TMPDIR = testRoot;
		try {
