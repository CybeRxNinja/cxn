...
import { resolveUpdateMethodForTest } from "@cxn/pi-coding-agent/cli/update-cli";
import { removeSyncWithRetries } from "@cxn/pi-utils";

// Issue #845: on Windows with Bun installed via Scoop, ~/.bun is a junction
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the cxn path that
...
// does not follow filesystem links, so it misclassified Bun-installed cxn
// as "binary" and tried to swap cxn.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies cxn reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies cxn at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "cxn");
