...
describe("getShellConfig", () => {
	it("directs invalid custom shell paths to the canonical config file", () => {
		const missingShell = path.join(os.tmpdir(), `cxn-missing-shell-${process.pid}`, "bash");
...
describe("resolveWindowsShell", () => {
	...
	function makeGitRoot(): string {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-git-root-"));
		tempDirs.push(root);
	...
	it("finds Git Bash in the default scoop app dir via USERPROFILE", () => {
		const profile = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-profile-"));
	...
	it("prefers a Git for Windows install root over the cmd.exe fallback", () => {
		const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-programfiles-"));
