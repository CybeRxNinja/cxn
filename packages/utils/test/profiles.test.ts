...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfileEnv = process.env.CXN_PROFILE;
		...
		configDir = `.cxn-profile-test-${Snowflake.next()}`;
	...
	afterEach(async () => {
		...
		if (originalOmpProfileEnv === undefined) {
			delete process.env.CXN_PROFILE;
		} else {
			process.env.CXN_PROFILE = originalOmpProfileEnv;
		}
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "cxn", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "cxn", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "cxn", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "cxn", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "cxn", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "cxn", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		// Fresh install: XDG vars are set (typical Linux) but no $XDG/cxn exists yet.
		...
		// Later, the base XDG app dir materializes (e.g. via `cxn config init-xdg`
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "cxn"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "cxn"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "cxn"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid CXN profile");
		expect(() => setProfile("work/team")).toThrow("Invalid CXN profile");
	});
	...
	it("does not restore a profile-derived agent dir as the default baseline", () => {
		// Reproduces a child process that inherited CXN_PROFILE=work plus the
		...
		// Re-snapshot exactly as module load would, now that CXN_PROFILE and the
...
describe("profile env + name validation", () => {
	it("honors CXN_PROFILE precedence and treats empty/default as the default profile", () => {
		// CXN_PROFILE is canonical and wins over the legacy PI_PROFILE fallback.
		expect(resolveProfileEnv("work", "other")).toBe("work");
		// PI_PROFILE is consulted only when CXN_PROFILE is undefined.
		expect(resolveProfileEnv(undefined, "work")).toBe("work");
		// An explicitly-empty CXN_PROFILE selects the default profile; it must NOT
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid CXN profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid CXN profile");
...
describe("dirs module import behavior", () => {
	...
	it("exposes worker-host without loading agent env", async () => {
		...
		try {
			const workerHostUrl = import.meta.resolve("@cxn/pi-utils/worker-host");
			const agentDir = path.join(root, "agent");
			await fs.mkdir(agentDir, { recursive: true });
			await Bun.write(path.join(agentDir, ".env"), "CXN_WORKER_HOST_PROBE=from-agent-env\n");
			const probePath = path.join(root, "probe.ts");
			await Bun.write(
				probePath,
				[
					...
					"process.stdout.write(JSON.stringify({",
					"	envProbe: process.env.CXN_WORKER_HOST_PROBE ?? null,",
			...
			delete childEnv.CXN_WORKER_HOST_PROBE;
			const proc = Bun.spawn([process.execPath, probePath], {
				stdout: "pipe",
				stderr: "pipe",
				env: childEnv,
	...
	it("ignores inherited profile agent dir when CXN_PROFILE explicitly selects default", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-utils-dirs-default-profile-"));
		const probeConfigDir = `.cxn-default-profile-${Snowflake.next()}`;
		try {
			const dirsUrl = url.pathToFileURL(path.join(import.meta.dir, "..", "src", "dirs.ts")).href;
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		const profileConfigDir = `.cxn-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "cxn", "profiles", "work");
