...
describe("document conversion cache directory", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfile = process.env.CXN_PROFILE;
	...
	afterEach(async () => {
		...
		restoreEnv("CXN_PROFILE", originalOmpProfile);
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/cxn exists", async () => {
		...
		expect(getDocumentConversionCacheDir()).toBe(
			path.join(process.env.XDG_CACHE_HOME, "cxn", "cache", "document-conversions"),
		);
	});
...
describe("test directory state cleanup", () => {
	it("restores the active profile from the current env after setAgentDir mutations", () => {
		...
		const originalOmpProfile = process.env.CXN_PROFILE;
		const originalPiProfile = process.env.PI_PROFILE;
		const originalXdgCacheHome = process.env.XDG_CACHE_HOME;
		try {
			process.env.CXN_PROFILE = "cache-profile";
...
describe("legacy file adoption on XDG paths", () => {
	...
	it("adopts legacy files at the XDG paths without clobbering existing XDG files", async () => {
		...
		await fs.mkdir(path.join(xdgState, "cxn"), { recursive: true });
		await fs.mkdir(path.join(xdgData, "cxn"), { recursive: true });
		// Legacy layout: key under ~/.cxn/agent, registry under ~/.cxn.
		await fs.mkdir(path.join(tempRoot, ".cxn", "agent"), { recursive: true });
		await fs.writeFile(path.join(tempRoot, ".cxn", "agent", "secret-placeholder.key"), "legacy-key");
		await fs.writeFile(path.join(tempRoot, ".cxn", "marketplaces.json"), '{"legacy":true}');
		// The XDG registry is already populated: adoption must not overwrite it.
		await fs.writeFile(path.join(xdgData, "cxn", "marketplaces.json"), '{"xdg":true}');
		...
		expect(key).toBe(path.join(xdgState, "cxn", "secret-placeholder.key"));
		expect(registry).toBe(path.join(xdgData, "cxn", "marketplaces.json"));
	...
	it("keeps the legacy paths canonical when XDG is inactive", async () => {
		if (process.platform === "win32") return;
		activateTempHome({});
		expect(getSecretPlaceholderKeyPath()).toBe(path.join(tempRoot, ".cxn", "agent", "secret-placeholder.key"));
		expect(getMarketplacesRegistryPath()).toBe(path.join(tempRoot, ".cxn", "marketplaces.json"));
	});
});
