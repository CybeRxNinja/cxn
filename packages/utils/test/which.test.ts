...
describe("$which", () => {
	...
	it.skipIf(process.platform === "win32")("uses the current process PATH for each cached lookup", () => {
		const firstDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-which-first-"));
		const secondDir = fs.mkdtempSync(path.join(os.tmpdir(), "cxn-which-second-"));
		tempDirs.push(firstDir, secondDir);

		const command = `cxn-which-${process.pid}`;
