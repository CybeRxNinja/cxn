/**
 * Set the OS-visible process name (`/proc/self/comm`) so `cxn` shows up as
 * `cxn` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target cxn (and `pkill bun` becomes
