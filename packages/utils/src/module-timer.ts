 ...
 * - **Dev runs only.** In the compiled `cxn` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("cxn.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("cxn.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("cxn.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("cxn.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
