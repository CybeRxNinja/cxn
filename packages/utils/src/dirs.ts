/**
 * Centralized path helpers for cxn config directories.
 *
 * Uses PI_CONFIG_DIR (default ".cxn") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/cxn/. This requires running `cxn config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, cxn trusts that the migration has been done.
 */
...
/** App name (e.g. "cxn") */
export const APP_NAME: string = "cxn";

/** Config directory name (e.g. ".cxn") */
export const CONFIG_DIR_NAME: string = ".cxn";
...
/** Default User-Agent header string (e.g. "cxn/17.2.12") */
export const USER_AGENT = `cxn/${VERSION}`;

/** Minimum Bun version */
export const MIN_BUN_VERSION: string = engines.bun.replace(/[^0-9.]/g, "");

const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;
const PROFILE_ENV_KEYS = ["CXN_PROFILE", "PI_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@cxn/pi-utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid CXN profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the two profile env vars. `CXN_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `CXN_PROFILE` is undefined. An
 * explicitly-empty `CXN_PROFILE` therefore selects the default profile rather
 ...
 */
export function resolveProfileEnv(cxn: string | undefined, pi: string | undefined): string | undefined {
	return normalizeProfileName(cxn !== undefined ? cxn : pi);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.CXN_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * CXN_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid CXN profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".cxn" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".cxn/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all cxn directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/cxn/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/cxn/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/cxn once `cxn config init-xdg` has migrated
		...
		// profile could resolve to `~/.cxn/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/cxn exists yet) and then silently move to
		// `$XDG_*_HOME/cxn/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.cxn/agent/sessions → $XDG_DATA_HOME/cxn/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `CXN_PROFILE` explicitly selected the
 ...
 * `~/.cxn/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.cxn). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.CXN_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.cxn/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.cxn). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.cxn/*)
// =============================================================================

/** Get the reports directory (~/.cxn/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.cxn/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get this process's dated log path (~/.cxn/logs/cxn.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.cxn/plugins or its XDG equivalent).
 *
 ...
 * (PR checkout, task isolation) and cleanup (`cxn worktree`). A relative value
 * would resolve against whatever cwd happened to launch `cxn`, so checkout and
 ...
 * isolation, and `cxn worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `CXN_WORKTREE_DIR` or the `~/.cxn/wt` default.
 *
 ...
 * `CXN_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.cxn/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return resolveWorktreeBase(process.env.CXN_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data");
}

/** Get the SSH control socket directory (~/.cxn/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.cxn/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.cxn/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.cxn/agent/python-gateway; XDG default: $XDG_STATE_HOME/cxn/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.cxn/puppeteer). */
export function getPuppeteerDir(): string {
	return dirs.rootSubdir("puppeteer", "cache");
}

/** Get the browser relay extension install directory (~/.cxn/browser-relay). */
...
/** Get the auto-QA grievances SQLite database path (~/.cxn/autoqa.db; XDG: $XDG_DATA_HOME/cxn/autoqa.db). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.cxn/wt/`).
...
/** Get the path to a single worktree directory (~/.cxn/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.cxn/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.cxn/cache/github-cache.db).
 * Honors the `CXN_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.CXN_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/**
 * Get the encrypted auth-broker snapshot cache path (~/.cxn/cache/auth-broker-snapshot.enc).
 * Honors the `CXN_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.CXN_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.cxn/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.cxn/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.cxn/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.cxn/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.cxn/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.cxn/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.cxn/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.cxn/autoresearch/<encoded-project>/runs/<runId>). */
...
/** Get the last-seen-changelog-version marker file (~/.cxn/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.cxn/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.cxn/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/cxn/cache/document-conversions). */
 ...
 * persisted transcripts). The legacy file is left in place for older cxn
...
/** Get the secret placeholder key path (~/.cxn/agent/secret-placeholder.key; XDG default: $XDG_STATE_HOME/cxn/secret-placeholder.key). Adopts a legacy key on first XDG resolution. */
...
/** Get the daemon runtime directory for a project (~/.cxn/run/daemons/<hash>; XDG default: $XDG_STATE_HOME/cxn/run/daemons/<hash>). */
...
/** Get the provider in-flight root directory (~/.cxn/run/provider-inflight; XDG default: $XDG_STATE_HOME/cxn/run/provider-inflight). */
export function getProviderInFlightRoot(): string {
	return dirs.rootSubdir(path.join("run", "provider-inflight"), "state");
}

/** Get the marketplaces registry path (~/.cxn/marketplaces.json; XDG default: $XDG_DATA_HOME/cxn/marketplaces.json). Adopts a legacy registry on first XDG resolution. */
...
/**
 * Persistent per-install UUID stored at `~/.cxn/install-id`.
 *
 ...
 * `~/.cxn/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.cxn/install-id`) regardless of the
