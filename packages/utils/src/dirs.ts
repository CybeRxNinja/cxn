 ...
 *
 * Exported so consumers of `@cyberxninja-omp/pi-utils/dirs` (CLI bootstrap, tests,
...
/** Plugin lock file (~/.omp/plugins/cxn-plugins.lock.json). */
 ...
 * fall back to `CXN_WORKTREE_DIR` or the `~/.omp/wt` default.
 *
...
/** Get the crash log path (~/.omp/agent/cxn-crash.log). */
export function getCrashLogPath(agentDir?: string): string {
	return dirs.agentSubdir(agentDir, "cxn-crash.log", "state");
}

/** Get the debug log path (~/.omp/agent/cxn-debug.log). */
