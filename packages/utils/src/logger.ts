/**
 * Centralized logger for cxn.
 *
 * Default: rotating `~/.cxn/logs/cxn.<DATE>.<PID>.log`, no console output (writing
 ...
 *
 * Each entry includes `process.pid` so concurrent cxn instances stay
 * traceable.
 */
...
const PROCESS_LOG_PATTERN = /^cxn\.(\d{4}-\d{2}-\d{2})\.(\d+)\.log(?:\.(\d+))?$/;
const PROCESS_AUDIT_PATTERN = /^\.cxn\.(\d+)-audit\.json$/;
...
function makeFileTransport(dir?: string): RotatingFileSink {
	...
	return new RotatingFileSink({
		...
		maxFiles: 5,
		auditFile: path.join(logsDir, `.cxn.${process.pid}-audit.json`),
	});
}
