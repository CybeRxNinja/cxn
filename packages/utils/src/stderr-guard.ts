 ...
 * crash restore). Unlike codex we redirect to the cxn log file — not
...
export interface SuppressTerminalStderrOptions {
	/** Redirect target path; defaults to today's cxn log file, then /dev/null. */
	redirectPath?: string;
...
export function suppressTerminalStderr(options?: SuppressTerminalStderrOptions): boolean {
	...
	try {
		...
		// on a fresh profile ~/.cxn/logs may not exist yet. Create it here so
