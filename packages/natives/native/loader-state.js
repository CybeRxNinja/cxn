...
/**
 * Native addon loader for `@cxn/pi-natives`.
 *
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "cxn"))) {
		return path.join(xdgDataHome, "cxn", "natives");
	}
	return path.join(os.homedir(), ".cxn", "natives");
}

function resolveLeafPackageDir(platformTag) {
	try {
		const require_ = createRequire(import.meta.url);
		return path.dirname(require_.resolve(`@cxn/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * into the per-version cache directory (`~/.cxn/natives/<version>/`) before loading.
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `cxn`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@cxn/pi-natives/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent cxn
...
// A concurrently starting older CXN binary creates or refreshes this directory
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	if (residentSentinel && diskHasExpectedSentinel) {
		...
		throw new Error(
			`Loaded ${candidate}, which exposes the @cxn/pi-natives@${residentVersion} version ` +
				`sentinel \`${residentSentinel}\` but not the @${ctx.packageVersion} sentinel ` +
				`\`${ctx.versionSentinelExport}\` this loader expects. cxn was upgraded to ` +
				`${ctx.packageVersion} while this session was running; the ${residentVersion} addon is ` +
				"still resident in this process. Disk is already consistent — restart cxn to pick up " +
				`${ctx.packageVersion} (reinstalling changes nothing).`,
		);
	}
	throw new Error(
		`Loaded ${candidate} but it does not expose the @cxn/pi-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @cxn/pi-natives\n" +
...
export function initLoaderContext(overrides = {}) {
	...
	const userDataDir =
		platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "cxn")
