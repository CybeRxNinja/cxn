...
/**
 * Native addon loader for `@cyberxninja-omp/pi-natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@cyberxninja-omp/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * `node_modules/@cyberxninja-omp/pi-natives/native/`, leaving an old binary next to a
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	if (residentSentinel && diskHasExpectedSentinel) {
		...
		throw new Error(
			`Loaded ${candidate}, which exposes the @cyberxninja-omp/pi-natives@${residentVersion} version ` +
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @cyberxninja-omp/pi-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @cyberxninja-omp/pi-natives\n" +
