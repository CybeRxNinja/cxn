 ...
 * build-time native artifacts), the compiled `cxn` binary fails to load
 ...
 * `execDir`. On WSL with `~/.local/bin/cxn` and no sibling `.node` file, this
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		const versionedDir = "/home/u/.cxn/natives/14.5.2";
		...
		// these are where the embedded-addon extraction lands (~/.cxn/natives/<v>) and where
		// `cxn update` writes the standalone binary on linux (~/.local/bin).
	...
	it("prefers platform leaf package candidates ahead of core nativeDir candidates on npm installs", () => {
		const leafPackageDir = "/app/node_modules/@cxn/pi-natives-linux-x64";
		const nativeDir = "/app/node_modules/@cxn/pi-natives/native";
		const candidates = resolveLoaderCandidates({
			addonFilenames: getAddonFilenames({ tag: "linux-x64", arch: "x64", variant: "baseline" }),
			isCompiledBinary: false,
			leafPackageDir,
			...
			versionedDir: "/home/u/.cxn/natives/15.5.15",
	...
	it("keeps Windows staging ahead of leaf package and core nativeDir candidates", () => {
		const versionedDir = "/home/u/.cxn/natives/15.5.15";
		const leafPackageDir = "/app/node_modules/@cxn/pi-natives-win32-x64";
		const nativeDir = "/app/node_modules/@cxn/pi-natives/native";
		const candidates = resolveLoaderCandidates({
			...
			execDir: "/app/node_modules/.bin",
			versionedDir,
			userDataDir: "/home/u/AppData/Local/cxn",
		});
