...
describe("zod-like parsing", () => {
	it("exposes callable omptype schemas with JSON Schema metadata", () => {
		...
		expect(schema({ website: "https://cxn.sh" })).toEqual({
			name: "Ada",
			website: "https://cxn.sh",
		});
	...
	it("supports string and number refinements plus nullable and optional values", () => {
		expect(z.string().regex(/^cxn$/).url().safeParse("cxn").success).toBe(false);
		expect(z.string().url().parse("https://cxn.sh")).toBe("https://cxn.sh");
