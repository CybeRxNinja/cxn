...
description: Use when creating a new cxn hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
```ts
import type { HookAPI } from "@cxn/pi-coding-agent/extensibility/hooks";

export default function myHook(cxn: HookAPI): void {
  cxn.on("tool_call", async (event, ctx) => {
...
```ts
cxn.on("tool_result", async (event, ctx) => {
...
```ts
cxn.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(cxn: HookAPI): void {
  cxn.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(cxn: HookAPI): void {
  cxn.on("tool_result", async (event) => {
...
export default function contextFilter(cxn: HookAPI): void {
  cxn.on("context", async (event) => {
