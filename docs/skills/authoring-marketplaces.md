...
A marketplace is a Git repository (or local directory) that contains a catalog file at either `.cxn-plugin/marketplace.json` (preferred for cxn-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
```json
{
  ...
  "plugins": [
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/cxn-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
1. Create `marketplace.json` at `.cxn-plugin/marketplace.json` (cxn-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
