  ...
  my-provider:
    ...
    models:
      - id: some-model-id
        ...
        reasoning: false
        input: [text]
        imageInputDecoder: stb # local STB decoder; CXN converts WebP before dispatch
        cost:
          input: 0
          output: 0
          cacheRead: 0
...
providers:
  openai:
    ...
    headers:
      X-Team-Key: "!bw get password cxn-team-key"
```
...
`disableStrictTools` is a provider-level flag that applies to all models in the provider. It disables the Anthropic `strict` marker only for tools that CXN would otherwise mark strict; it does not change runtime tool argument validation. CXN can automatically retry without strict tools after Anthropic reports a strict-grammar-too-large error before the first streamed token, but proxies that reject the `strict` field for other reasons should set this flag explicitly.
...
The built-in vLLM provider can be pointed at a non-default endpoint without declaring a custom discovery type. CXN uses vLLM's `/v1/models` metadata and preserves vLLM's `max_model_len` field as the discovered context window.
