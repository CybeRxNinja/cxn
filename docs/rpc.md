...
`security://` is reserved for CXN's producer-neutral software-security resource
...
The bundled [`cxn-rpc`](../python/cxn-rpc/pyproject.toml) distribution provides the process-backed Python client. Its import package is `cxn_rpc`; the package API, typed commands and events, host-tool/host-URI helpers, and orchestration examples are maintained in the [`cxn-rpc` README](../python/cxn-rpc/README.md).
