...
`cxn` resolves settings from built-in defaults, a persistent global config file, optional project-local config, one-shot CLI overlays, and in-memory runtime overrides. Reach for project settings when one repository needs a different provider set, model role, tool policy, memory backend, or UI behavior than your global defaults — without touching your machine-wide configuration.

Settings are stored as plain YAML mappings. Every key, its type, default, and enum values come from the settings schema. `cxn config` exposes the complete schema; the interactive `/settings` panel exposes the schema entries that have UI metadata.
...
- For instruction files discovered into the agent context (`AGENTS.md`, `.cxn/`, etc.), see [Context files](./context-files.md).
...
| ----------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Global            | `~/.cxn/agent/config.yml` (or existing `config.yaml`) | The main persistent settings file. `config.yml` is the canonical write target; an existing `config.yaml` is loaded and updated in place. | `/settings`, `cxn config set`, and `cxn config reset` write here.                                                                                                                |
| Global legacy     | `~/.cxn/agent/settings.json`                          | Migrated into `config.yml` once, only when neither main YAML filename exists.                                                            | Not written after migration; the original is renamed to `settings.json.bak`.                                                                                                     |
| Project           | `<cwd>/.cxn/config.yml` (plus `.cxn/settings.json`)   | Loaded when the process working directory has a non-empty `.cxn/`.                                                                       | Settings commands do not write arbitrary project keys. With `modelRoleStorage: project`, model-selector role assignments update only `modelRoles` here; edit other keys by hand. |
| Project legacy    | `<cwd>/.cxn/settings.json`                            | Still read; project `config.yml` is merged on top of it.                                                                                 | Not written by settings commands.                                                                                                                                                |
...
`PI_CODING_AGENT_DIR` relocates the `~/.cxn/agent` base directory. When it is set, the global `config.yml`, the auth store (`agent.db`), and everything else under the agent directory move with it. Use `cxn config path` to print the active agent directory.

Native project settings are intentionally scoped to the process working directory's `.cxn/` folder — settings discovery does **not** walk ancestor directories looking for the nearest `.cxn/`. Other discovery providers (Claude, Codex, Gemini, Cursor, OpenCode) can also contribute project-level settings from their own files; those are read-only from `cxn` settings commands and can be turned off by provider id (see [Provider and source disabling](#provider-and-source-disabling)).
...
- A settings YAML file whose top level is not a mapping is invalid. On writable startup, `cxn` moves an invalid persistent settings file to a uniquely named `.broken-*` backup and exits with the original error and backup path. A `--config` overlay with a bare array/scalar is also a hard error, but is not moved.

## Reading and writing settings

Use the interactive `/settings` panel inside a session, or the `cxn config` command from a shell. Both read merged effective settings. Ordinary persistent writes land in the **global** file; model-selector role changes are the exception when `modelRoleStorage: project` (see [Where writes go](#where-writes-go)).

```bash
cxn config list                 # all settings with current effective values
cxn config list --json          # same, machine-readable
cxn config get theme.dark       # one value
cxn config get theme.dark --json
cxn config set compaction.enabled false
cxn config set defaultThinkingLevel medium
cxn config reset steeringMode   # restore a key to its schema default
cxn config path                 # print the active agent directory
```

For users who want the full first-run animation on normal launches, set `startup.showSplash`:

```bash
cxn config set startup.showSplash true
```
...
| Command                        | Effect                                                                                                                                                                                                                                                                                            |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cxn config list`              | Print every setting grouped by tab, with its current value and type. `--json` emits an object keyed by setting path with `{ value, type, description }`. Configured credential fields are masked as `********` in human output; in JSON their `value` is omitted and `redacted: true` is emitted. |
| `cxn config get <key>`         | Print the effective value of one key. Unknown keys exit non-zero. `--json` emits `{ key, value, type, description }`. This is an explicit single-key request, so credential values are returned unmasked.                                                                                         |
| `cxn config set <key> <value>` | Parse `<value>` against the key's schema type and write it to the global main YAML file.                                                                                                                                                                                                          |
| `cxn config reset <key>`       | Write the key's schema **default** back to the global config (this persists the default, it does not delete the key).                                                                                                                                                                             |
| `cxn config path`              | Print the active agent directory (honors `PI_CODING_AGENT_DIR`).                                                                                                                                                                                                                                  |
| `cxn config init-xdg`          | On Linux and macOS, create the `cxn` directories under the effective XDG data, state, and cache homes. It does not move existing files or set the XDG environment variables. Other platforms exit non-zero.                                                                                       |

`cxn config` with no subcommand, `--help`, or `-h` lists settings. The `--json` flag is accepted by `list`, `get`, `set`, and `reset`.

### Value parsing

`cxn config set` parses the value string according to the target key's schema type. The string is trimmed first.
...
`cxn config set`, `cxn config reset`, `/settings`, and ordinary runtime settings changes write the global main YAML file under the active agent directory. They do not write arbitrary keys to `<cwd>/.cxn/config.yml`. The one supported project write path is a model-selector role assignment when `modelRoleStorage` is `project`; it updates only that role under `<cwd>/.cxn/config.yml`, and missing project roles continue to fall back to global roles. To create any other project-local override, edit the project file directly (see [Project-local config](#project-local-config)). Saves are debounced and re-read the file under a lock, so external edits made while a session is open are preserved.
...
3. **Project settings** — `<cwd>/.cxn/settings.json` then `<cwd>/.cxn/config.yml` (and contributions from other discovery providers at project level).
4. **Global settings** — `~/.cxn/agent/config.yml`.
...
| `CXN_AUTH_BROKER_URL`   | `auth.broker.url`           | Env value takes precedence over config.                                                           |
| `CXN_AUTH_BROKER_TOKEN` | `auth.broker.token`         | Env value takes precedence over config.                                                           |
...
Create `<repo>/.cxn/config.yml` when a repository needs its own settings:
...
```bash
cxn --config ./local/ci-settings.yml "check this failure"
cxn --config ./base.yml --config ./experiment.yml "try this model"
```
...
# <repo>/.cxn/config.yml — inside this repo ONLY groq is disabled
disabledProviders:
  - groq
```
...
Every key below is defined in the settings schema; `cxn config list` shows the full set with current values. Defaults and enum values are taken from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
| `modelRoleStorage`     | enum    | `global`                    | `global` saves model-selector role assignments in the active global/profile config; `project` saves only those role assignments in `<cwd>/.cxn/config.yml`. Missing project roles fall back to global roles.                                                                                                                                                                                                     |
...
A value of `-1` means "use the provider/model default" — `cxn` does not send that parameter.
...
| `tools.approval`               | record  | `{}`    | Per-tool policy keyed by tool name; each value is `allow`, `deny`, or `prompt`. e.g. `cxn config set tools.approval '{"bash":"prompt"}'`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `lsp.shared`                      | boolean | `true`    | Share one language server per project across local `cxn` processes through the daemon broker; falls back to private servers when the broker is unavailable. |
...
| `autolearn.enabled`           | boolean | `false`       | Experimental: after the agent stops, nudge it to capture lessons to memory and create/enhance isolated managed skills under `~/.cxn/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active). |
...
`compaction` has additional tuning keys (idle compaction, supersede/drop heuristics) visible in `cxn config list`. See [Compaction](./compaction.md) for the full strategy reference.
...
| `providers.maxInFlightRequests`     | record  | `{}`      | Positive per-provider concurrency limits for LLM HTTP requests, shared across local `cxn` processes using the same config root. Omitted providers are unlimited. `cxn config set` rejects non-positive or non-numeric values.                                                                                                                                                                                                          |
...
| `auth.broker.url`                   | string  | _(unset)_ | Auth-broker URL. Overridden by `CXN_AUTH_BROKER_URL`.                                                                                                                                                                                                                                                                                                                                                                                  |
| `auth.broker.token`                 | string  | _(unset)_ | Auth-broker token. Overridden by `CXN_AUTH_BROKER_TOKEN`.                                                                                                                                                                                                                                                                                                                                                                              |
...
`cxn config list` exposes many more grouped settings, including: `task.*` (subagent concurrency, isolation, model overrides), `skills.*` and `commands.*` (discovery toggles), `mcp.*`, `github.*`, `async.*`, `goal.*`, `loop.*`, `todo.*`, `magicKeywords.*`, `ttsr.*` (time-traveling stream rules), `display.*`, `startup.*`, `share.*`, `collab.*`, `stt.*`/`tts.*`, `memories.*`/`hindsight.*`/`mnemopi.*` (memory backends), and `bashInterceptor.*`. Each follows the same type/default rules shown above.

## Legacy migration

`cxn` migrates older config shapes automatically. None of these require action; they are listed so you know what changes you may see in `config.yml`.

### Startup migration to `config.yml`

When neither `~/.cxn/agent/config.yml` nor the compatible `config.yaml` exists, startup builds canonical `config.yml` once from legacy sources, then writes the result:

1. `~/.cxn/agent/settings.json` (renamed to `settings.json.bak` after a successful parse).
...
- Start `cxn` from the directory that contains `.cxn/config.yml`. Settings discovery only checks the current working directory's `.cxn/`, not ancestor directories.
- Ensure `.cxn/` is non-empty; empty config directories are ignored.
- Confirm the file is valid YAML and its top level is a mapping.
- Run `cxn config get <key>` from that directory to see the effective value.
...
### `cxn config set` changed the wrong file

`cxn config set` and `cxn config reset` always write the global `config.yml` under the active agent directory. Run `cxn config path` to print it. For project-local settings, edit `<repo>/.cxn/config.yml` directly.

### `cxn config reset` did not remove my key

`reset` writes the schema **default** value into the global config — it persists the default rather than deleting the key. To stop overriding a project value from global config, delete the key from `~/.cxn/agent/config.yml` by hand.
...
### `cxn config set <key>` says "Unknown setting"

Keys must match a schema path exactly, with no shorthand. Use `theme.dark`, not `theme`. Run `cxn config list` to see every valid key.
