...
Each Python kernel is a single subprocess: `<resolved-python> -u <runner.py>`. The runner is bundled with the host binary (Bun text import), written to an `cxn-python-runner` cache under the OS temp directory once per script hash, and reused by subsequent spawns.
...
Status events the prelude emits (e.g. `_emit_status("find", count=…)`) ship inside display bundles under `application/x-cxn-status` so the existing TUI status renderer keeps working.
...
2. Managed venv at `~/.cxn/python-env`
...
- `application/x-cxn-status` MIME inside `display` → structured status events
...
- `application/x-cxn-status` → status events
...
- **No Python on PATH** — Install a system Python 3.10+ or place a compatible venv at `~/.cxn/python-env`. `cxn setup python --check` reports the resolved interpreter.
