...
- Internal Rust crates remain covered by native architecture docs unless promoted as standalone user-facing commands or APIs. The contributor-facing map lives at [`native-crates.md`](./native-crates.md); today every `crates/*` entry is internal to `@cyberxninja-omp/pi-natives` and the embedded shell, so [`natives-architecture.md`](./natives-architecture.md) and the surrounding native docs own them.
...
- Inputs/storage: configuration comes from `python/robomp/.env` and the mounted `~/.omp/agent/models.container.yml`; GitHub webhook events feed a SQLite-backed queue. The Compose deployment persists the database, per-issue worktrees, session transcripts, and logs in the `robcxn_data` volume under `/data`.
...
- Package: `@cyberxninja-omp/omp-stats`; bin: `omp-stats`; main user path: `omp stats`.
...
- Package: public `@cyberxninja-omp/omptype`; install with `bun add @cyberxninja-omp/omptype`; requires Bun 1.3.14 or newer.
...
- Public surfaces: `@cyberxninja-omp/omptype` for native authoring, `/typebox` and `/zod` for compatibility builders, and `/ark` for the alias-free ArkType compatibility facade.
...
- Package: private `@cyberxninja-omp/typescript-edit-benchmark`; support library with no standalone bin.
...
- Package: private `@cyberxninja-omp/pi-metaharness`; bin: `metaharness`.
...
- Package: private `@cyberxninja-omp/browser-relay`; user command: `omp browser-relay`.
...
- Package: private `@cyberxninja-omp/collab-web`; production client: <https://my.omp.sh/>.
...
- Package: public `@cyberxninja-omp/snapcompact`; install with `bun add @cyberxninja-omp/snapcompact`; requires
  Bun 1.3.14 or newer.
...
- Runtime constraint: rasterization and PNG encoding require `@cyberxninja-omp/pi-natives`.
...
- Package: public `@cyberxninja-omp/pi-mnemopi`; bin: `mnemopi`; requires Bun 1.3.14 or newer. Install globally with `bun add --global @cyberxninja-omp/pi-mnemopi`, then run `mnemopi <command>`. From a source checkout, `bun packages/mnemopi/src/cli.ts <command>` runs the same entrypoint.
