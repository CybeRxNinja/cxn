...
This document explains how token/tool streaming is normalized in `@cyberxninja-omp/pi-ai`, then propagated through `@cyberxninja-omp/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@cyberxninja-omp/pi-ai`
