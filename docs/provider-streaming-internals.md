...
This document explains how token/tool streaming is normalized in `@cxn/pi-ai`, then propagated through `@cxn/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@cxn/pi-ai`
