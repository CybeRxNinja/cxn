...
┌───────────────────────────────────────┐
│ 1) ~/.cxn/agent, ~/.claude, ...       │
│ 2) <cwd>/.cxn, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .cxn before user .cxn;
...
- CXN native: `~/<PI_CONFIG_DIR>/agent` (normally `~/.cxn/agent`; a named profile changes this as described below)
- `~/.claude`
- `~/.codex`
- `~/.gemini`
...
`CONFIG_DIR_NAME` is `.cxn` (`packages/utils/src/dirs.ts`). `PI_CONFIG_DIR` changes the CXN user root used by the generic helpers. `PI_CODING_AGENT_DIR` is different: for the default profile it changes `getAgentDir()` consumers such as native discovery, settings, and runtime state, but it does **not** change the generic `getConfigDirs()` / `findConfigFile()` CXN base. Named profiles ignore `PI_CODING_AGENT_DIR`.

## Profiles

A named profile (`cxn --profile <name>`, `CXN_PROFILE`, or the legacy fallback `PI_PROFILE`) relocates the CXN user base. `CXN_PROFILE` wins when it is defined, including when it is explicitly empty; `default`, empty, or whitespace selects the default profile. When a profile is active, every CXN-native user-level path written here as `~/.cxn/agent/...` normally resolves to `~/.cxn/profiles/<name>/agent/...`. `--alias <command>` does not select a profile by itself: paired with `--profile`, it creates a shell shortcut for that profile.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own CXN config, never the default profile's agent config.

Keybindings are the one exception: a named profile merges the default profile's `~/.cxn/agent/keybindings.*` under its own `~/.cxn/profiles/<name>/agent/keybindings.*`, with the profile file overriding per binding ([#4867](https://github.com/can1357/oh-my-pi/issues/4867)). Keybindings describe the terminal/keyboard in front of the user, which doesn't change with the active profile, so user-level remaps keep working in every profile unless the profile explicitly overrides them. The inherited file is read-only for the profile process — legacy-format migration of the default profile's file only happens when the default profile itself runs.

On macOS and Linux, an existing `$XDG_DATA_HOME/cxn`, `$XDG_STATE_HOME/cxn`, or `$XDG_CACHE_HOME/cxn` can relocate the corresponding data, state, or cache paths. For a named profile, CXN uses an XDG category only when that category already contains `cxn/profiles/<name>`; otherwise that category remains under `~/.cxn/profiles/<name>`. Run `cxn config init-xdg` before relying on XDG paths.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.cxn`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.cxn/agent` as shorthand for the active profile's agent directory unless an environment override or XDG path is being discussed.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.cxn`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: the first present file among `~/.cxn/agent/config.yml` and `config.yaml`
2. Project settings: discovered via the settings capability (`settings.json` and `config.yml` from providers)
3. Config overlays: `PI_CONFIG_FILES` (platform path-list), followed by repeated `cxn --config <path>` files; all are loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.cxn/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.cxn` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.cxn/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.cxn/agent/skills`, without requiring the root `.cxn` directory itself to be non-empty.
- `SYSTEM.md`, `RULES.md`, and `.cxn/AGENTS.md` read user-level files directly and use the nearest non-empty ancestor `.cxn` directory for project files. `RULES.md` becomes an always-apply sticky rule. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract.
- MCP does not use the non-empty-root admission helper. It reads project `.cxn/mcp.json` then `.cxn/.mcp.json`, followed by user `mcp.json` then `.mcp.json`, directly.

### Scope-specific loading

- Skills: `<ancestor>/.cxn/skills/*/SKILL.md` and `~/.cxn/agent/skills/*/SKILL.md`
...
- Context files: `.cxn/AGENTS.md`; standalone ancestor `AGENTS.md` files are loaded separately by the low-priority `agents-md` provider

### Nearest-project lookup nuance

For `SYSTEM.md`, `RULES.md`, and `.cxn/AGENTS.md`, the native provider walks upward to the nearest non-empty project `.cxn` directory.
...
```text
# ~/.cxn/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery checks the current project directory bases first (`<cwd>/.cxn`, `.claude`, `.codex`, `.gemini`), then the user bases in the generic helper order. Unlike native `SYSTEM.md`, project title discovery does **not** walk ancestor directories.
