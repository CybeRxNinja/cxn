...
- Tool calls render through the `<cxn-tool-view>` web component — the React per-tool renderers shared with collab-web (`packages/collab-web/src/tool-render/`), prebuilt into `src/export/html/tool-views.generated.js` by `bun run gen:tool-views`.
...
The interactive TUI's `loadCustomShare()` checks `~/.cxn/agent` for the first existing candidate:
...
4. Upload target is chosen by `share.store`:
   - **Share server** (default, `store: "blob"`) — `POST <share.serverUrl>`
     (default `https://my.cxn.sh/s`) with the raw blob, capped at 1 MB.
...
Use `--prompt-cache-key <key>` to pin the provider prompt-cache identity explicitly and independently from both the CXN session id and `--provider-session-id`. `--provider-session-id` continues to control provider session/routing headers and sticky credential selection; `--prompt-cache-key` controls the OpenAI Responses `prompt_cache_key` payload where supported.
...
- `/resume @claude` and `/resume @codex` open a foreign-session picker. Selecting one converts and persists it under a fresh CXN session identity, then switches to that new session.
