# MCP configuration in CXN

This guide explains how to add, edit, and validate MCP servers for the CXN coding agent.
...
CXN can discover MCP servers from multiple tools (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, and more), but for CXN-native configuration you should usually use one of these primary files:

- Project: `.cxn/mcp.json`
- User: `~/.cxn/agent/mcp.json` (or `~/.cxn/profiles/<name>/agent/mcp.json` when a named profile is active — see [Profiles](#profiles))

The native provider also reads `.cxn/.mcp.json` and `~/.cxn/agent/.mcp.json` for compatibility, but CXN writes to the primary `mcp.json` paths above.

CXN also accepts fallback standalone files in the project root:

- `mcp.json`
- `.mcp.json`

Use `.cxn/mcp.json` or `~/.cxn/agent/mcp.json` when you want CXN to own the configuration. Use root `mcp.json` / `.mcp.json` only when you want a portable fallback file that other MCP clients may also read.

### Imported tool configs

CXN also translates these current tool-native sources:
...
- installed Claude marketplace plugins and CXN extension packages that declare MCP servers

For Claude Code, Codex, Gemini CLI, Cursor, and Windsurf, the project entry is encountered before its same-named user entry — matching CXN-native config, whose project entry precedes its active-profile user entry — so a project `enabled: false` suppresses a same-named user server. OpenCode currently encounters the user entry first. Cross-provider priority is listed in [Discovery and precedence](#discovery-and-precedence).

### Profiles

Named profiles (`cxn --profile <name>`, the `--alias` shortcut, or `CXN_PROFILE`/`PI_PROFILE`) isolate user-level MCP config. When a profile is active, the **user** scope resolves to the profile's agent directory instead of the default one:

- Default profile: `~/.cxn/agent/mcp.json`
- Profile `<name>`: `~/.cxn/profiles/<name>/agent/mcp.json`

Discovery, the `/mcp` commands, and the config writer all follow the active profile, so a profile sees **only** its own user-level servers — never the default profile's `~/.cxn/agent/mcp.json`. Add a server to a profile by launching under it (`cxn --profile <name>`) and running `/mcp add` → User level, or by editing `~/.cxn/profiles/<name>/agent/mcp.json` directly.

Project-scoped MCP config (`.cxn/mcp.json`) is keyed to the working directory, not the profile, so it applies under every profile. External-tool configs (`.claude/`, `.cursor/`, etc.) are also profile-independent because they belong to those tools rather than to an CXN profile.

MCP follows the same profile rules as the rest of CXN-native config; see [Configuration Discovery → Profiles](./config-usage.md#profiles).
...
CXN now writes this automatically when `/mcp add`, `/mcp enable`, `/mcp disable`, `/mcp reauth`, or other config-writing flows create or update an CXN-managed MCP file.

## File shape

CXN supports this top-level structure:
...
The config writer accepts names up to 100 characters containing letters, numbers, `_`, `-`, `.`, and `:`. The bundled schema currently omits `:` from its name pattern, so an CXN-managed namespaced plugin entry such as `cloudflare:cloudflare-api` may be valid at runtime while an editor reports a schema error.
...
- `requestIdFormat?: "number" | "string"` — outgoing JSON-RPC request-id encoding; defaults to per-transport integers. `"string"` uses collision-resistant snowflake IDs. This CXN-specific field is read only from CXN-native files, root `mcp.json` / `.mcp.json`, and CXN extension packages; configs translated from other tools ignore it.
...
`CXN_MCP_TIMEOUT_MS` has process-wide precedence over every per-server `timeout`. Set it to `0` to disable client-side timeouts, or to a positive millisecond value such as `120000`. If it is unset or invalid, CXN uses the server value and then the 30-second default; invalid values are logged and ignored.
...
CXN understands two auth-related objects.
...
For managed OAuth, `auth` tells CXN how to find and refresh a stored credential. Although `"apikey"` is an accepted `type`, it does not load or inject an API key from auth storage. Put API keys directly in stdio `env` or remote `headers` (prefer an environment-variable or `!command` indirection described below).

You normally do not need to write this block: when CXN completes an OAuth flow for an `http`/`sse` server, it stores the credential under a deterministic id derived from the active profile and server URL (`mcp_oauth:profile:<profile>:<url>`), with the refresh material embedded. Any
...
still honored when it resolves; if it points at another profile's row, CXN falls
...
`prompt` controls the OAuth `prompt` authorization parameter. By default CXN omits it, except that a requested `offline_access` scope defaults to `"consent"` so the provider can issue refresh access. Set it explicitly to a provider-supported value such as `"consent"` or `"select_account"`, or to `""` to force omission.
...
CXN expands `${VAR}` and `${VAR:-default}` placeholders while discovering MCP configs from CXN-native files and standalone fallback files. Expansion applies recursively to string values in `command`, `args`, `env`, `cwd`, `url`, `headers`, `auth`, and `oauth`; unresolved placeholders remain literal strings.
...
Before CXN launches a stdio server or makes an HTTP/SSE request, it resolves stdio `env` values and HTTP/SSE `headers` values like this:

1. If a value starts with `!`, CXN runs the rest as a shell command with a 10s timeout and uses trimmed stdout. Successful results are cached for the lifetime of the process.
2. If the command fails, times out, or prints only whitespace, that `env`/`headers` entry is omitted.
3. Otherwise CXN checks whether the whole value names an environment variable.
4. If that environment variable is set to a non-empty value, CXN uses the environment value; otherwise it uses the string literally.
...
`/mcp enable` and `/mcp disable` update `enabled` directly when the definition is in an CXN-owned writable file. CXN does not mutate another tool's config: for such sources, those commands maintain the user-level allowlist or denylist instead and remove a conflicting stale override.
...
- If you paste a remote server config and forget `"type": "http"`, CXN will treat it as `stdio` and complain that `command` is missing
- `sse` remains valid for compatibility, but new hosted servers should usually be configured as `http`

## Discovery and precedence

CXN loads providers in descending priority. The MCP-capable order is:

1. CXN native config
2. CXN extension packages
3. Claude Code
...
Within CXN native config, project `.cxn/mcp.json` precedes `.cxn/.mcp.json`, then the active profile's user `mcp.json` and `.mcp.json`. Root fallback `mcp.json` precedes root `.mcp.json`. In practice:

- prefer `.cxn/mcp.json` or the active profile's user `mcp.json` for an CXN-specific override
...
Pick one transport. CXN treats `command` as stdio and `url` as http/sse.
...
### The server exists in another tool's config but not in CXN

Run `/mcp list`. CXN discovers many third-party MCP files, but project-level loading can also be disabled via the `mcp.enableProjectConfig` setting, and a user-level `disabledServers` entry can suppress a server by name.
...
Malformed JSON or a missing/invalid server map makes that provider contribute no entries from the file; depending on the provider, CXN records a discovery warning or logs the parse failure rather than failing the session. Correct the JSON shape, then run `/mcp reload` and `/mcp list`.
