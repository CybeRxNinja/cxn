...
The compiled macOS `cxn` binaries shipped on GitHub Releases can be signed with a
...
- `curl https://cxn.sh/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/cxn-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/cxn-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/cxn-signing             # upload all five
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/cxn-darwin-arm64
```
