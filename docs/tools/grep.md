...
4. Internal URLs are resolved before filesystem scope resolution:
   ...
   - `cxn://` expands to every embedded documentation file via URL completion;
...
9. It calls native `grep()` from `@cxn/pi-natives` with:
...
5. **Internal URL paths**
   ...
   - `cxn://` expands to all embedded documentation files so it can be used as a docs search root.
