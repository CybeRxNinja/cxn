...
- Filesystem
  ...
  - `pr_checkout` may create worktree directories named `<pr-number>-<repo-hash>` under the base selected by `CXN_WORKTREE_DIR`, then `worktree.base`, then the profile/XDG-aware default (normally `~/.omp/wt`), and add git worktrees there.
