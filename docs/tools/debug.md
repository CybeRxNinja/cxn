...
- Subprocesses / native bindings
  ...
  - `getWorkProfile(30)` comes from `@cyberxninja-omp/pi-natives`.
  - CPU profiling uses `node:inspector/promises`; heap snapshots use `Bun.generateHeapSnapshot("v8")`; raw/log viewers sanitize text via `sanitizeText()` from `@cyberxninja-omp/pi-utils`.
