...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@cyberxninja-omp/pi-coding-agent`
  - `@mariozechner/pi-agent-core` → `@cyberxninja-omp/pi-agent-core`
  - `@mariozechner/pi-tui` → `@cyberxninja-omp/pi-tui`
  - `@mariozechner/pi-ai` → `@cyberxninja-omp/pi-ai`
  - `@mariozechner/pi-utils` → `@cyberxninja-omp/pi-utils`
  - `@mariozechner/pi-catalog` → `@cyberxninja-omp/pi-catalog`
  - `@mariozechner/pi-natives` → `@cyberxninja-omp/pi-natives`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@cyberxninja-omp/pi-coding-agent`, and so on).
- The bare `typebox` package is not an `@cyberxninja-omp/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@cyberxninja-omp/pi-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@cyberxninja-omp/pi-natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@cyberxninja-omp/pi-natives` | Native implementation with a small TS wrapper |
...
| `formatSize` from `pi-coding-agent`                                    | `formatBytes` from `@cyberxninja-omp/pi-utils`                                                                                                                                                                                                                                                      |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@cyberxninja-omp/pi-natives`
