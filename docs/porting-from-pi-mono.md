...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@cxn/pi-coding-agent`
  - `@mariozechner/pi-agent-core` → `@cxn/pi-agent-core`
  - `@mariozechner/pi-tui` → `@cxn/pi-tui`
  - `@mariozechner/pi-ai` → `@cxn/pi-ai`
  - `@mariozechner/pi-utils` → `@cxn/pi-utils`
  - `@mariozechner/pi-catalog` → `@cxn/pi-catalog`
  - `@mariozechner/pi-natives` → `@cxn/pi-natives`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@cxn/pi-coding-agent`, and so on).
- The bare `typebox` package is not an `@cxn/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@cxn/pi-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@cxn/pi-natives` exports generated native bindings from `packages/natives/native`.
...
| `ctx.ui.addAutocompleteProvider()`          | Wired in interactive mode; no-op stubs in ACP/RPC/headless contexts | Factory wrapping matches upstream; cxn's editor has no custom `triggerCharacters`, so wrapped providers surface at the built-in trigger points |
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@cxn/pi-natives` | Native implementation with a small TS wrapper |
...
| `pkg.pi` manifest field                                                | `pkg.cxn` preferred; fallback to `pkg.pi` remains                                                                                                                                                                                                                                            |
| `StringEnum` from `pi-ai`                                              | `Type.Enum` from `pi.typebox`, or `pi.arktype.enumerated(...)`; `pi-ai` no longer exports `StringEnum`                                                                                                                                                                                       |
| `formatSize` from `pi-coding-agent`                                    | `formatBytes` from `@cxn/pi-utils`                                                                                                                                                                                                                                                      |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@cxn/pi-natives`
