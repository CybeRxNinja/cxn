...
```text
Config sources (.cxn/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.cxn/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.cxn/mcp.json` (project) or `~/.cxn/agent/mcp.json` (user) for explicit control.
...
CXN-native MCP config (`.cxn/mcp.json`, `~/.cxn/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`. `requestIdFormat` accepts only `"number"` or `"string"`; other values warn and fall back to numeric IDs.
...
block is optional. CXN honors an explicit arbitrary or legacy credential ID when
...
block—CXN looks for a credential under deterministic IDs derived from the
...
If no credential resolves, CXN connects without injecting an OAuth value.
Refresh or credential-resolution failures are logged; when possible, CXN
