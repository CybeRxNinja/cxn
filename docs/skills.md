...
- `description` is required for:
  - native `.cxn` provider skill discovery (`requireDescription: true`)
  - `cxn-plugins` extension-package skills and the `github` provider (`.github/skills/`), which also pass `requireDescription: true`
...
3. **Managed (auto-learn) skills** (`cxn-managed` provider) resolved dead-last, so any same-named enabled authored skill from a provider or custom directory takes precedence
...
1. `native` (priority 100) — `.cxn` user/project skills via `src/discovery/builtin.ts`
2. `cxn-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:`, `--extension`/`-e`, or installed plugins under `~/.cxn/plugins/node_modules`
...
7. `cxn-managed` (priority 5) — auto-learn skills under `~/.cxn/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
...
The `agents` provider (`.agent[s]/skills`) is the canonical CXN-native location and has its own `enableAgentsUser`/`enableAgentsProject` toggles — disabling Claude/Codex/Pi does **not** turn it off. Providers without a dedicated toggle (`claude-plugins`, `opencode`, `github`, …) are enabled if **any** named third-party source toggle is enabled.
