...
cxn plugin <npm/link action> ...
  ...
  -> mutate user plugins data root {package.json,node_modules,cxn-plugins.lock.json}
...
- `cxn-plugins.lock.json` — runtime state for npm/link/marketplace plugins:
...
- [`src/discovery/cxn-plugins.ts`](../packages/coding-agent/src/discovery/cxn-plugins.ts) — conventional capability discovery for npm/link extension packages
