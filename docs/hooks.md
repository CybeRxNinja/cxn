...
- JS/TS hook factories discovered through `hookCapability` (for example `.cxn/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
...
`packages/coding-agent/src/extensibility/hooks/index.ts` and the package subpath `@cxn/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@cxn/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
