...
```ts
import type { HookAPI } from "@cyberxninja-omp/pi-coding-agent/extensibility/hooks";
...
`packages/coding-agent/src/extensibility/hooks/index.ts` and the package subpath `@cyberxninja-omp/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@cyberxninja-omp/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
