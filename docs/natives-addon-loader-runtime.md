...
- `nativesDir`, normally `~/.cxn/natives`; it uses `$XDG_DATA_HOME/cxn/natives` only when `$XDG_DATA_HOME/cxn` exists;
- `versionedDir`: `<nativesDir>/<packageVersion>`;
- legacy compiled-binary directory: `%LOCALAPPDATA%/cxn` (or `~/AppData/Local/cxn`) on Windows, `~/.local/bin` elsewhere;
...
1. Every selected filename in `@cxn/pi-natives-<tag>`.
