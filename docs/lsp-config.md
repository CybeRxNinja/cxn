# LSP configuration in CXN

This guide explains how to configure language servers for the CXN coding agent.
...
When no config file contributes a server override, CXN auto-detects built-in servers by intersecting two conditions:
...
CXN merges LSP config from multiple sources, lowest to highest precedence:
...
|            | Cwd config dirs: `<cwd>/.cxn/lsp.*`, `<cwd>/.claude/lsp.*`, `<cwd>/.codex/lsp.*`, `<cwd>/.gemini/lsp.*`      |
...
The native user config directory follows `PI_CONFIG_DIR` and active profiles; `~/.cxn/agent/lsp.json` is the default-profile spelling. This shared config lookup does not use `PI_CODING_AGENT_DIR` as an arbitrary replacement base. Project and cwd sources do not walk ancestors.

**Recommended locations:**

- User-wide preferences → active native agent directory's `lsp.json`
- Project-specific overrides → `<cwd>/.cxn/lsp.json`

> **Note:** Auto-detection mode is skipped only when at least one readable config contributes a non-empty server map. A config that only sets `idleTimeoutMs` still uses built-in auto-detection. With server overrides, CXN first merges them onto all defaults, then keeps servers whose root markers match the cwd, whose binary resolves, and whose merged config is not `disabled`.
...
The `capabilities` object enables optional server-specific features that CXN supports on a per-server basis:
...
Place the override in `<project>/.cxn/lsp.json`:
...
The user-level config in `~/.cxn/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
