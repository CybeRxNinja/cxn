...
The SDK is the in-process integration surface for `@cyberxninja-omp/pi-coding-agent`.
...
```bash
bun add @cyberxninja-omp/pi-coding-agent
```
...
The package root, `@cyberxninja-omp/pi-coding-agent`, is the complete embedding surface. It includes `createAgentSession` and the focused `/sdk` exports, plus lower-level session, auth, model, mode, extension, and tool APIs.
...
```ts
import { createAgentSession, SessionManager } from "@cyberxninja-omp/pi-coding-agent";
...
```ts
import { SessionManager } from "@cyberxninja-omp/pi-coding-agent";
...
```ts
import type { AgentSession } from "@cyberxninja-omp/pi-coding-agent";
