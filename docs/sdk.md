...
The SDK is the in-process integration surface for `@cxn/pi-coding-agent`.
...
The package root, `@cxn/pi-coding-agent`, is the complete embedding surface. It includes `createAgentSession` and the focused `/sdk` exports, plus lower-level session, auth, model, mode, extension, and tool APIs.
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.cxn/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@cxn/pi-coding-agent";
...
```ts
import { SessionManager } from "@cxn/pi-coding-agent";
...
```ts
import type { AgentSession } from "@cxn/pi-coding-agent";
