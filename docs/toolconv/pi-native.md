...
`cxn auth-gateway`. It is **not a textual tool-call dialect**: there is no
...
provider credentials—for example, a containerized cxn talking to a host
...
`baseUrl` MUST identify an `cxn auth-gateway` (or compatible service). Missing
`baseUrl` fails with:
