...
- **ID → name parsing differs between references.** The official
  ...
  while vLLM and cxn take the last dot-separated segment before the colon.
...
- **Streaming marker splits.** Section and call markers can be split across token boundaries.
  ...
  argument fragments. cxn's owned scanner also holds partial markers, but buffers a
