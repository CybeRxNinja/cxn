"""Host tools exposed to the agent through `cxn_rpc.host_tool`.
...
from cxn_rpc import HostTool, HostToolContext, RpcCommandError, host_tool
...
class AbortController:
    ...
    teardown of the cxn subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching cxn. Carries the abort-task signal
...
def ensure_workspace_dependencies(bindings: ToolBindings) -> None:
    ...
    (``@cxn/pi-*``) only exist after an install, so without one any
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent cxn teardown.
