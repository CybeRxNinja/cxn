...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.cxn-session/`, `context/`, `artifacts/`, `.cxn-tmp/`, `.cxn-xdg`):
   single-owner. Owned by the active slot UID/GID (`cxn-N`) when slot
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.cxn-tmp`` (symlink-protection: a user
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    xdg_root = workspace.root / ".cxn-xdg"
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.cxn-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.cxn-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".cxn-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".cxn-session"
