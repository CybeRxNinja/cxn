...
pytestmark = pytest.mark.skipif(
    os.environ.get("ROBCXN_PERMISSION_E2E") != "1",
    reason="set ROBCXN_PERMISSION_E2E=1 to run slot-permission e2e tests",
)
...
def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to cxn-N UIDs")
...
def _share_tree_with_slots(path: Path) -> None:
    for root, dirs, files in os.walk(path):
        root_path = Path(root)
        os.chown(root_path, 0, _SHARED_CXN_GID)
        root_path.chmod(0o2770)
        for dirname in dirs:
            child = root_path / dirname
            os.chown(child, 0, _SHARED_CXN_GID)
            child.chmod(0o2770)
...
def _prepare_shared_cargo_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    for path in (cargo_home, cargo_target):
        ...
        os.chown(path, 0, _SHARED_CXN_GID)
        path.chmod(0o2770)
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".cxn-xdg" / "cache" / "bun-install" / "root-owned-stale"
    ...
    assert (cargo_target / "debug").stat().st_gid == _SHARED_CXN_GID
