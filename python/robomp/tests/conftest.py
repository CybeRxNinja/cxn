...
# block carrying the `__ROBCXN_CONFIG__` sentinel. The real Vite-built bundle
...
_PLACEHOLDER_INDEX_HTML = (
    ...
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="robomp-config" type="application/json">__ROBCXN_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBCXN_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBCXN_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBCXN_BOT_LOGIN": "robomp-bot",
        "ROBCXN_GIT_AUTHOR_NAME": "robomp-bot",
        "ROBCXN_GIT_AUTHOR_EMAIL": "robomp-bot@example.invalid",
        "ROBCXN_REPO_ALLOWLIST": "octo/widget",
        "ROBCXN_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBCXN_THINKING": "high",
        "ROBCXN_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBCXN_SQLITE_PATH": str(tmp_path / "robomp.sqlite"),
        "ROBCXN_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBCXN_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBCXN_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBCXN_NATIVES_CACHE_ENABLED": "false",
        ...
        "ROBCXN_ISSUE_INDEX_SYNC_SECONDS": "0",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBCXN_PROVIDER", raising=False)
    monkeypatch.setenv("ROBCXN_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBCXN_GH_PROXY_URL", None)
    baseline.pop("ROBCXN_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBCXN_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBCXN_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBCXN_PROVIDER", raising=False)
    monkeypatch.setenv("ROBCXN_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
